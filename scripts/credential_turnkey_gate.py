...
* A row that can only be proven with LIVE credentials reports FAIL marked
  ...
  PASS.
* Live-readiness CLAIMS (``connection_validated`` / ``sandbox_validated`` /
...
def _check_secret_safe(evidence: dict) -> RowVerdict:
    ...
    Fail-closed on absence of evidence too: when the canonical secret scan did
    not run, secret safety cannot be asserted, so the row FAILS rather than
    degrading to an advisory — a security row never warns its way to green.
    """
    ...
    return _fail("secret scan did not run; secret-safety cannot be asserted")
...
def _check_live_readiness_evidence(evidence: dict) -> RowVerdict:
    ...
    any provider claiming connection_validated / sandbox_validated / partner_live
...
def _collect_certification(evidence: dict, backend: Path) -> None:
    ...
    high = {"connection_validated", "sandbox_validated", "partner_live"}
    dishonest = []
    for p in providers.values():
        if p["state"] in high:
...
def _collect_manifests(evidence: dict) -> None:
    ...
    provider manifest registry and the integration-contract layer."""
    try:
    ...
    # Main's manifest model has no transport_protocol/base_url_config fields:
    # non-REST transport is declared by a manifest's webhook surface, and any
    # public base URLs the deployment requires.
    evidence["transport_declared"] = any(
        bool(m.webhooks.supported)
        or bool(m.webhooks.registration_supported)
        or bool(m.deployment.required_public_urls)
        for m in manifests
    )
    # Main declares idempotency semantics at the record level, not in the
    # manifest model: RawProviderRecord and AetherEvent derive an
    # idempotency_key (tenant|provider_identity|provider_record_id|version) so
    # ingestion replays dedupe instead of double-applying.
    from shared.integration_contracts.events import AetherEvent, RawProviderRecord
    evidence["idempotency_declared"] = (
        hasattr(RawProviderRecord, "idempotency_key")
        and hasattr(AetherEvent, "idempotency_key")
    )
