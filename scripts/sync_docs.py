...
def write_automation() -> None:
    ...
    lines += [
        ...
        "- `make verification-disposition EXECUTE=1` — the single blocking normal-PR authority, run once after finalization; it runs universal-fast plus the Impact Graph's affected checks and emits one disposition artifact.",
        ...
        "",
        "- Keep implementation and blueprint work in a draft PR. Focused checks are local feedback; do not run aggregate gates or dispatch hosted PR CI for every intermediate push.",
        ...
        "- After integration, clarification review, gap remediation, documentation, and source-linked review, mark the PR ready for review. The `ready_for_review` event starts the single normal PR authority: `make verification-disposition BASE=<base> EXECUTE=1`. "
        ...
        "disposition` status for finalized pull requests (`ready_for_review`); the retired legacy full "
        "gate remains available through local, trusted-main, nightly, and release "
        "commands but is not a PR job.",
        "- `.github/workflows/repo-health.yml` keeps finalized-PR docs/size signals advisory "
