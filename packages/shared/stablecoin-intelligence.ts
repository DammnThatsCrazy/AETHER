...
export type StablecoinSupportCapability =
  | 'send'
  | 'receive'
  | 'hold'
  | 'deposit'
...
export interface StablecoinSupportAssertion extends StablecoinTenantScoped {
  ...
  deployment_id: string;
  capability: StablecoinSupportCapability;
...
export const STABLECOIN_CAPABILITIES: readonly StablecoinSupportCapability[] = [
...
export function validateStablecoinSupportAssertion(input: unknown): StablecoinValidationResult {
  ...
  requireObservationOnly(input, errors);
  if (!STABLECOIN_CAPABILITIES.includes(input.capability as StablecoinSupportCapability)) {
