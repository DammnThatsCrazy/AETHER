...
export const outcomeTypeRegistryContractVersion = '1.0.0' as const;

/** Outcome domains a type may belong to (sorted). */
export const outcomeTypeDomains = [
  'agentic',
  'commercial',
  'economic',
  'fraud',
...
export type OutcomeTypeDomain = typeof outcomeTypeDomains[number];

/** Registered outcome types (sorted). */
export const outcomeTypeIds = [
...
export type OutcomeTypeId = typeof outcomeTypeIds[number];

/** One registered outcome type (mirrors the registry schema). */
export interface OutcomeTypeDescriptor {
  id: OutcomeTypeId;
  domain: OutcomeTypeDomain;
  name: string;
  description: string;
}

export const outcomeTypeDefinitions: Record<OutcomeTypeId, OutcomeTypeDescriptor> = {
  access_control_enforced: {
