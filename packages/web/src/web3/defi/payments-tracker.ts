// =============================================================================
// AETHER SDK — PAYMENT/STREAMING TRACKER
// Superfluid, Sablier, LlamaPay, Request Network, Utopia Labs
// Events: create_stream, cancel_stream, withdraw_stream, create_vesting
// =============================================================================

import type { VMType, DeFiCategory } from '../../types';
import { identifyProtocol } from './protocol-registry';

export interface PaymentsTrackerCallbacks {
  onInteraction: (data: Record<string, unknown>) => void;
  onPositionChange: (data: Record<string, unknown>) => void;
}

export class PaymentsTracker {
  private callbacks: PaymentsTrackerCallbacks;
  private readonly category: DeFiCategory;

  constructor(callbacks: PaymentsTrackerCallbacks) {
    this.callbacks = callbacks;
    this.category = 'payments' as DeFiCategory;
  }
