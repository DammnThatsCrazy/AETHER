// =============================================================================
// AETHER SDK — CEX ON-CHAIN TRACKER
// Binance, Coinbase, Kraken, OKX, Bybit, KuCoin, Gate.io, Huobi, Gemini, Bitfinex
// Events: cex_deposit, cex_withdrawal
// =============================================================================

import type { VMType, DeFiCategory } from '../../types';
import { identifyProtocol } from './protocol-registry';

export interface CexTrackerCallbacks {
  onInteraction: (data: Record<string, unknown>) => void;
  onPositionChange: (data: Record<string, unknown>) => void;
}

export class CexTracker {
  private callbacks: CexTrackerCallbacks;
  private readonly category: DeFiCategory;

  constructor(callbacks: CexTrackerCallbacks) {
    this.callbacks = callbacks;
    this.category = 'cex' as DeFiCategory;
  }
