// =============================================================================
// AETHER SDK — ROUTER/AGGREGATOR TRACKER
// 1inch, Paraswap, 0x, CowSwap, Jupiter, Li.Fi, Socket, KyberSwap, OpenOcean, Odos
// Events: route_swap, split_trade, cross_chain_swap
// =============================================================================

import type { VMType, DeFiCategory } from '../../types';
import { identifyProtocol } from './protocol-registry';

export interface RouterTrackerCallbacks {
  onInteraction: (data: Record<string, unknown>) => void;
  onPositionChange: (data: Record<string, unknown>) => void;
}

export class RouterTracker {
  private callbacks: RouterTrackerCallbacks;
  private readonly category: DeFiCategory;

  constructor(callbacks: RouterTrackerCallbacks) {
    this.callbacks = callbacks;
    this.category = 'router' as DeFiCategory;
  }
