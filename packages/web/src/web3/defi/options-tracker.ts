// =============================================================================
// AETHER SDK — OPTIONS TRACKER
// Lyra, Dopex, Premia, Hegic, Opyn, Friktion, Zeta Markets
// Events: buy_option, sell_option, exercise, expire, settle
// =============================================================================

import type { VMType, DeFiCategory } from '../../types';
import { identifyProtocol } from './protocol-registry';

export interface OptionsTrackerCallbacks {
  onInteraction: (data: Record<string, unknown>) => void;
  onPositionChange: (data: Record<string, unknown>) => void;
}

export class OptionsTracker {
  private callbacks: OptionsTrackerCallbacks;
  private readonly category: DeFiCategory;

  constructor(callbacks: OptionsTrackerCallbacks) {
    this.callbacks = callbacks;
    this.category = 'options' as DeFiCategory;
  }
