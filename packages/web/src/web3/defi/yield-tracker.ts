// =============================================================================
// AETHER SDK — YIELD AGGREGATOR TRACKER
// Yearn V3, Beefy, Harvest, Convex, Concentrator, Sommelier, Kamino, Tulip, Francium
// Events: vault_deposit, vault_withdraw, harvest_yield, compound, auto_compound
// =============================================================================

import type { VMType, DeFiCategory } from '../../types';
import { identifyProtocol } from './protocol-registry';

export interface YieldTrackerCallbacks {
  onInteraction: (data: Record<string, unknown>) => void;
  onPositionChange: (data: Record<string, unknown>) => void;
}

export class YieldTracker {
  private callbacks: YieldTrackerCallbacks;
  private readonly category: DeFiCategory;

  constructor(callbacks: YieldTrackerCallbacks) {
    this.callbacks = callbacks;
    this.category = 'yield' as DeFiCategory;
  }
