...
describe('SemanticContextCollector', () => {
  it('uses the canonical sessionId and eventId passed in (no self-minting)', () => {
    const collector = new SemanticContextCollector('0.1.0-alpha.0');
  ...
  it('stamps sdk version and platform', () => {
    const ctx = new SemanticContextCollector('0.1.0-alpha.0').collect('s', 'e');
    expect(ctx.sdkVersion).toBe('0.1.0-alpha.0');
