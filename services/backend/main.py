...
from services.agent_access_intelligence.authority_routes import (
    authorizations_router as aai_authorizations_router,
    policy_router as aai_capability_policy_router,
)
from services.agent_access_intelligence.declaration_routes import (
    capability_declarations_router as aai_capability_declarations_router,
)
...
def create_app() -> FastAPI:
    ...
    app.add_middleware(
        ...
        # Per-PR frontend previews (staging only); None leaves it off.
        allow_origin_regex=preview_origin_regex(
            settings.api.cors_preview_origin_suffix, settings.env.value
        ),
    ...
    app.include_router(aai_authorizations_router)       # /v1/capability-authorizations (tenant)
    ...
    # Kyber Graph. Mounted after the workforce plane because every one of its
    # routes resolves an authenticated Kyber context, and the tenant-scoped
    # routes additionally require a live access scope established through
    # kyber_scopes_router.
    app.include_router(kyber_graph_router)
    # Tenant Mirror. Reads tenant data only through the scoped graph gateway, so
    # it is mounted after it for the same reason: every route needs an active
