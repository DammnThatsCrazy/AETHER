"""LEDGER M2 — append-only hash chain over ``bronze_sdk_events``.

Exercises the per-tenant tamper-evidence chain that ``ingest_many`` populates on
every NEW Bronze row, run against the ``AETHER_ENV=local`` in-memory backend so
no Postgres is required. Covers:

* new rows get a populated ``integrity_hash``;
* ``prev_hash`` links consecutive rows (chain head is NULL);
* ``hash_chain.verify_chain`` over a tenant's rows passes;
...
def _outbox(tenant: str, event_id: str) -> OutboxEvent:
    return OutboxEvent(
        tenant_id=tenant,
        event_id=event_id,
...
def _tenant_rows(tenant: str) -> list[dict]:
    return [r for r in _bronze_store().values() if r.get("tenant_id") == tenant]


def _chained_rows(tenant: str) -> list[dict]:
    """A tenant's post-cutover rows (those carrying an integrity_hash), in chain
    order. NULL-hash historical rows are the pre-cutover boundary and are not
    part of the verifiable chain."""
    rows = [r for r in _tenant_rows(tenant) if r.get("integrity_hash")]
    return sorted(rows, key=_chain_sort_key)


def _verify(tenant: str) -> dict:
    return hash_chain.verify_chain(
        _chained_rows(tenant),
        partition_key=_chain_partition,
        sort_key=_chain_sort_key,
        canonical_field_variants=lambda r: [_canonical_fields(r)],
...
async def test_new_rows_get_populated_integrity_hash_and_verify():
    reset_in_memory_stores()
    tenant = _tenant()

    await _ingest(tenant, ["e0", "e1", "e2"])

    rows = _chained_rows(tenant)
    assert len(rows) == 3
    # Every new row carries a real (hex sha256) integrity_hash.
...
async def test_prev_hash_links_consecutive_rows():
    reset_in_memory_stores()
    tenant = _tenant()
...
async def test_second_ingest_continues_chain_from_prior_batch():
    reset_in_memory_stores()
    tenant = _tenant()

    await _ingest(tenant, ["a1", "a2"])
    first_tail = max(_chained_rows(tenant), key=_chain_sort_key)["integrity_hash"]
    ...
    rows = _chained_rows(tenant)
    assert len(rows) == 3
    # b1 sorts last (later created_at; also "b1" > "a2"); it chains onto the
    # tail integrity_hash of the previous batch.
...
async def test_pre_cutover_null_rows_are_boundary_not_crash():
    reset_in_memory_stores()
    tenant = _tenant()

    # Simulate a historical row written before M2: hash columns are NULL.
    store = _bronze_store()
    hist_id = _bronze_id(tenant, "old", "2")
    store[hist_id] = {
        "id": hist_id,
        "tenant_id": tenant,
        "event_id": "old",
...
async def test_cross_tenant_chains_are_independent():
    ...
    a_rows = _chained_rows(tenant_a)
    b_rows = _chained_rows(tenant_b)
...
async def test_tampering_with_hashed_field_is_detected():
    reset_in_memory_stores()
    tenant = _tenant()

    await _ingest(tenant, ["e0", "e1", "e2"])

    # Tamper a hashed canonical field on the middle row.
    victim = next(r for r in _tenant_rows(tenant) if r["event_id"] == "e1")
