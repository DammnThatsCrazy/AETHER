...
def _cid(campaign: dict):
    """upsert_external_campaign / create_custom_campaign return the full
    campaign record (the production contract); extract the canonical id."""
...
class TestCanonicalCampaignFlow:
    def test_import_campaign_produces_canonical_uuid(self):
        ...
        cid = _cid(_run(svc.upsert_external_campaign(
            ...
            external_campaign_name="Holiday Season 2026",
            source_connector_id="connector-meta-001",
        )))
    ...
    def test_replay_source_events_no_duplicate_campaigns(self):
        svc = _make_registry()
        cid1 = _cid(_run(svc.upsert_external_campaign(TENANT, "google_ads", "acc", "g-001", external_campaign_name="Camp")))
        cid2 = _cid(_run(svc.upsert_external_campaign(TENANT, "google_ads", "acc", "g-001", external_campaign_name="Camp")))
        cid3 = _cid(_run(svc.upsert_external_campaign(TENANT, "google_ads", "acc", "g-001", external_campaign_name="Camp")))
        assert str(cid1) == str(cid2) == str(cid3)

    def test_rename_retains_uuid(self):
        svc = _make_registry()
        cid_v1 = _cid(_run(svc.upsert_external_campaign(TENANT, "meta_ads", "acc", "fb-500", external_campaign_name="Original Name")))
        cid_v2 = _cid(_run(svc.upsert_external_campaign(TENANT, "meta_ads", "acc", "fb-500", external_campaign_name="Renamed Campaign")))
        cid_v3 = _cid(_run(svc.upsert_external_campaign(TENANT, "meta_ads", "acc", "fb-500", external_campaign_name="Renamed Again")))
    ...
    def test_spend_resolved_to_canonical_uuid(self):
        ...
        cid = _cid(_run(svc.upsert_external_campaign(TENANT, "google_ads", "acc-1", "provider-camp-999", external_campaign_name="Spend Camp")))
    ...
    def test_touchpoint_with_utm_resolves_to_same_canonical_uuid(self):
        ...
        svc = _make_registry()
        resolver = _make_resolver()
        cid = _cid(_run(svc.upsert_external_campaign(TENANT, "google_ads", "acc-1", "g-camp-utm", external_campaign_name="UTM Camp")))
    ...
    def test_utm_id_alias_resolves_at_099_confidence(self):
        svc = _make_registry()
        resolver = _make_resolver()
        cid = _cid(_run(svc.upsert_external_campaign(TENANT, "google_ads", "acc", "g-utm-id-camp", external_campaign_name="Camp")))
    ...
    def test_cross_tenant_isolation_enforced(self):
        ...
        svc = _make_registry()
        resolver = _make_resolver()
        cid_a = _cid(_run(svc.upsert_external_campaign("tenant-A", "google_ads", "acc", "cross-camp", external_campaign_name="Camp A")))
...
class TestAmbiguityFlow:
    ...
    def test_resolve_review_enables_future_resolution(self):
        ...
        cid = _cid(_run(svc.create_custom_campaign(TENANT, name="TikTok Q3 Camp")))
