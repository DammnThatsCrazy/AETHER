...
`services/backend/services/traffic/metrics.py` (`METRIC_NAMES`) and emitted through the shared
...
|---|---|---|---|
| `classification_total` | counter | `source_class`, `proof_level` (one increment per dimension) | `services/backend/services/silver/dispatcher.py` (per resolved touchpoint) |
...
| `invalid_source_link_total` | counter | — | `services/backend/services/traffic/routes.py` redirect (unresolvable token) |
...
| `reclassification_throughput_total` | counter | — | `services/backend/services/traffic/repair.py` (rows reclassified) |
| `reclassification_failure_total` | counter | — | repair.py (per-run errors / hard failure) |
| `shadow_divergence_total` | counter | `diverged` ∈ {true, false} | `services/backend/services/traffic/shadow.py` shadow-compare |

## Hooks exposed for other agents

`services/backend/services/traffic/metrics.py` exposes named helpers so web/native agents emit
