...
class StablecoinPriceSink(Protocol):
    ...
    owns the durable write. Implementations MUST be idempotent on the snapshot
    identity (re-emitting the same snapshot must not duplicate) and MUST NOT
    change the snapshot's availability/price semantics.
    """
...
class StablecoinChainlinkPriceConnector(ConnectorCertificationMixin):
    ...
    async def get_price_observation(self, *, tenant_id: str = "") -> StablecoinPriceObservation:
        ...
        When a persistence sink is configured AND emit is enabled the snapshot is
        emitted through it (fail-open; the returned snapshot never changes).
        """
