    ...
    async def reconcile(
        ...
        if intent is None:
            ...
            result = ReconciliationResult(
                ...
                "onchain transaction reverted", dict(onchain.evidence),
            )
        elif onchain.finality_status != FinalityState.FINALIZED:
            result = ReconciliationResult(
                intent.tenant_id, intent.payment_intent_id,
                ReconciliationState.PENDING_FINALITY,
                "onchain transaction is not finalized", dict(onchain.evidence),
            )
        elif self._matches(intent, onchain):
            result = ReconciliationResult(
                intent.tenant_id, intent.payment_intent_id,
                ReconciliationState.MATCHED,
                "tenant intent matches finalized onchain evidence", dict(onchain.evidence),
            )
        elif self._partial(intent, onchain):
            result = ReconciliationResult(
                intent.tenant_id, intent.payment_intent_id,
                ReconciliationState.PARTIAL,
                "payer/recipient/deployment match but amount differs", dict(onchain.evidence),
            )
        else:
            result = ReconciliationResult(
                intent.tenant_id, intent.payment_intent_id,
                ReconciliationState.MISMATCHED,
                "tenant intent conflicts with onchain evidence", dict(onchain.evidence),
            )
