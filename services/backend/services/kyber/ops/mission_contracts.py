...
``services/agents/models/evidence.py``'s ``VerificationResult.decision`` —
``passed``/``failed``/``needs_review``/``inconclusive``). The backend never
imports the Agent Layer package (see ``services/backend/services/agent/worker_bridge.py``), so
...
#: ``VerificationResult.decision`` in ``services/agents/models/evidence.py``. Kept
