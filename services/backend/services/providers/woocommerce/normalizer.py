...
# Identity continuity: source identity creation for WooCommerce customers (via SourceIdentityRegistry)
...
_logger = _get_logger("aether.providers.woocommerce.identity")


async def register_woocommerce_customer_identity(
    tenant_id: str,
    source_system_id: str,
    customer_data: dict,
) -> object | None:
    """Register a source identity from a WooCommerce customer record.
    ...
    try:
        ...
        _logger.warning("woocommerce source identity registration failed: %s", e)
        return None
