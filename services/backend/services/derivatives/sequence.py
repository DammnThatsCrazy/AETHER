...
* Cooperative shutdown is handled at the worker level: main's
  ``ReconnectingStream`` has no intra-stream ``should_stop`` hook (the branch's
  ``VenueDerivativesAdapter.run_stream(..., should_stop=...)`` was not ported),
  so ``run_until_stopped`` checks ``should_stop()`` BETWEEN cycles and ends the
  loop cleanly. An ``asyncio.CancelledError`` mid-cycle propagates out of
  ``ReconnectingStream.run`` (re-raised, never swallowed); the next resume
  restarts from the last persisted cursor (at-least-once).
...
class SupervisedStreamWorker:
    ...
    ``adapter`` is any object exposing
    ``run_stream(*, resume_cursor=..., max_reconnects=..., market_id=...)
    -> StreamResult`` (the ``VenueDerivativesAdapter`` surface on main) and,
    optionally, ``last_stream_cursor() -> int|None`` for the next cursor.
    """
    ...
    async def run_once(
        ...
        *,
        ...
        ``should_stop`` is accepted for worker-topology compatibility but is NOT
        forwarded to the stream: main's ``ReconnectingStream`` carries no
        intra-stream stop hook (cooperative stop is checked BETWEEN cycles by
        ``run_until_stopped``).
        """
        cursor = await self.restore_cursor()
        result, next_sequence = await self._drive_stream(
            cursor, max_reconnects=max_reconnects, market_id=market_id,
        )
    ...
    async def _drive_stream(
        ...
        *,
    ...
    ) -> tuple[StreamResult, Optional[int]]:
        """Run one stream and return ``(result, next_cursor)``.

        PORT-ADAPT: main's ``VenueDerivativesAdapter.run_stream`` returns a
        :class:`StreamResult` but discards the :class:`ReconnectingStream`
        (whose ``tracker.expected_next`` IS the advanced cursor), and exposes
        no ``last_stream_cursor()``. So for a real main adapter we drive the
        canonical :class:`ReconnectingStream` directly (the exact stream the
        adapter would build) and read ``tracker.expected_next``. Adapters that
        still expose ``last_stream_cursor()`` (branch-style fakes) keep the
        adapter-driven path.
        """
        factory = getattr(self.adapter, "_stream_factory", None)
        if factory is None:
            # Credential-gated / unwired adapter: main's run_stream short-
            # circuits to an empty completed result; nothing to advance.
            if hasattr(self.adapter, "run_stream") and hasattr(self.adapter, "last_stream_cursor"):
                result = await self.adapter.run_stream(
                    resume_cursor=cursor,
                    max_reconnects=max_reconnects,
                    market_id=market_id,
                )
                return result, self.adapter.last_stream_cursor()
            return StreamResult(completed=True), cursor
        if hasattr(self.adapter, "run_stream") and hasattr(self.adapter, "last_stream_cursor"):
            result = await self.adapter.run_stream(
                resume_cursor=cursor,
                max_reconnects=max_reconnects,
            ...
            return result, self.adapter.last_stream_cursor()
        venue_id = getattr(self.adapter, "venue_id", self.connector_id)
        channel = (
            self.adapter.stream_channel()
            if callable(getattr(self.adapter, "stream_channel", None))
            else "account"
        )
        default_market = (
            f"{venue_id}:{getattr(self.adapter, '_deployment', 'mainnet')}:*"
        )
        stream = ReconnectingStream(
            ...
            channel=channel,
            max_reconnects=max_reconnects,
            sleeper=getattr(self.adapter, "_sleeper", None),
        )
        result = await stream.run(resume_cursor=cursor)
        return result, stream.tracker.expected_next
    ...
    async def run_until_stopped(
        ...
        *,
        ...
        return {
            ...
            # main's StreamResult has no `cancelled` flag; cancellation re-raises
            # out of run_once (never returns), so a returned summary is never a
            # cancelled one. getattr keeps branch-style results compatible.
            "cancelled": bool(last is not None and getattr(last, "cancelled", False)),
            "last_cursor": (
