    ...
    def capabilities(self) -> list[ReleaseCapability]:
        return [
            ReleaseCapability(
                ...
                status="partial",
                description="Agentic observations routed through the shared canonical Bronze→outbox→Silver→CanonicalActivity spine",
                completed_items=[
                    "Canonical ingest_observation bridge delegates to the shared spine (services/ingestion/bronze_bulk.ingest_many)",
                    "Typed Bronze + event_outbox written in one transaction on Topic.SDK_EVENTS_VALIDATED",
                    "Deterministic provider/integration/environment-namespaced idempotency event_id",
                    "Silver fact dispatch via SilverDispatcher/AgentExecutionProjector (silver_agent_execution_facts)",
                    "Durable outbox relay (services/ingestion/outbox_relay) drains event_outbox with lease/retry/dead-letter",
                    "ingest_observation scrubs sensitive keys before Bronze write",
                    "Delegation gated by AGENTIC_OBS_CANONICAL_SPINE_ENABLED (default off); flag-off path unchanged",
                ],
                blockers=[
                    "Canonical spine delegation is flag-gated (default off) and live only when OUTBOX_RELAY_ENABLED=true",
                    "External-account/budget route delegation and consent-gated observation ingest still in progress",
                    "Replay UX for failed agentic Bronze records relies on the shared relay and is not independently verified",
                ],
            ),
            ReleaseCapability(
