"""
Aether Agent Layer — Nous Controller
Top orchestration controller across all domain controllers.
Nous is an orchestrator, NOT a full-access worker.
...
logger = logging.getLogger("aether.nous")


class NousController:
    """
    Nous — top orchestration controller. Sits under Governance and
    ...
    def __init__(
        ...
        event_bus: EventBus,
        cycle: CycleRuntime | None = None,
    ):
        self.governance = governance
        self.objective_runtime = objective_runtime
        self.event_bus = event_bus
        self.cycle = cycle or CycleRuntime()
    ...
    def register_controller(self, name: str, controller: Any) -> None:
        ...
        logger.info(f"Nous: Registered domain controller '{name}'")
    ...
    def admit_objective(self, objective: Objective) -> bool:
        ...
        if self.governance.is_halted:
            logger.warning("Nous: Governance halt — rejecting objective")
            return False

        if not self.governance.can_admit_objective():
            logger.warning("Nous: Objective ceiling reached — rejecting")
            return False
        ...
        logger.info(
            f"Nous: Objective {objective.objective_id[:8]}... admitted "
    ...
    def execute_next_step(self, objective_id: str) -> Optional[dict[str, Any]]:
        ...
        if controller is None:
            logger.error(f"Nous: No controller '{decision.controller}' registered")
        ...
        try:
            ...
            logger.error(f"Nous: Step {next_step.step_id[:8]}... failed: {e}")
    ...
    def check_replan(self, objective_id: str) -> bool:
        ...
        if self.replanner.needs_replan(plan):
            ...
            new_plan.activate()
            logger.info(f"Nous: Replanned objective {objective_id[:8]}...")
            return True
        return False
    ...
    def complete_objective(self, objective_id: str) -> None:
        ...
        if obj:
            ...
            logger.info(f"Nous: Objective {objective_id[:8]}... completed")
    ...
    def health(self) -> dict[str, Any]:
        return {
            ...
            "cycle_state": self.cycle.summary(),
        }
