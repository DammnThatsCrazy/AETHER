 ...
 * Server First-Value Journey Snapshot Test
 *
 * Blueprint §3.4 — Server emits the canonical-first-value-journey fixture through
 * server-native/platform-idiomatic APIs and normalizes into the same canonical envelope.
 *
...
const PLATFORM_OVERRIDES = {
  platform_id: 'server',
  sdk_name: '@aether/server',
  device_platform: 'server',
  surface: 'server_backend',
};

describe('PR D: Cross-Platform Parity — Server First-Value Journey', () => {
  ...
  // Normalize all events with Server-specific overrides
  ...
  it('should map each Server event to the correct step event_type', () => {
  ...
  it('should have unique event_ids across all Server events', () => {
    const ids = normalizedEvents.map((e) => e.event_id);
    expect(new Set(ids).size).toBe(ids.length);
  });

  it('should have unique idempotency_keys across all Server events', () => {
  ...
  it('should have consistent consent status across all Server events', () => {
    for (const event of normalizedEvents) {
  ...
  it('should have consistent tenant fields across all Server events', () => {
    for (const event of normalizedEvents) {
  ...
  it('should have Server-specific platform and SDK fields after normalization', () => {
    for (const event of normalizedEvents) {
      expect(event.platform_id).toBe('server');
      expect(event.source.platform).toBe('server');
      expect(event.source.sdk).toBe('@aether/server');
      expect(event.source.environment).toBe('staging');
      expect(event.source.surface).toBe('server_backend');
      expect(event.sdk.name).toBe('@aether/server');
      expect(event.sdk.version).toBe('0.1.0-alpha.0');
      expect(event.device.platform).toBe('server');
    }
  });
  ...
  it('should preserve anonymous_id across all Server events', () => {
    for (const event of normalizedEvents) {
  ...
  it('should have monotonically increasing timestamps across all Server events', () => {
  ...
  it('should validate every Server event has a complete EventEnvelope shape', () => {
  ...
  it('should assert Server produces the same identity transition as all other platforms', () => {
  ...
  it('should assert Server uses the same idempotency key format as all other platforms', () => {
    const keys = normalizedEvents.map((e) => e.idempotency_key);
    expect(keys).toEqual(
  ...
  it('should assert Server produces the same schema hash as all other platforms', () => {
    expect(schema.schema_hash).toBe('sha256:canonical-first-value-journey-v1-2024-09-11');
  });

  it('should assert Server shares the same tenant/source/app/surface fields as all other platforms', () => {
    ...
    expect(firstEvent.source.platform).toBe('server');
    expect(firstEvent.source.surface).toBe('server_backend');
  });
});
