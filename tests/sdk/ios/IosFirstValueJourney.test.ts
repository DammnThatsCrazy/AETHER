 ...
 * iOS First-Value Journey Snapshot Test
 *
 * Blueprint §3.4 — iOS emits the canonical-first-value-journey fixture through
 * iOS-native/platform-idiomatic APIs and normalizes into the same canonical envelope.
 *
...
describe('PR D: Cross-Platform Parity — iOS First-Value Journey', () => {
  ...
  // Normalize all events with iOS-specific overrides
  ...
  it('should map each iOS event to the correct step event_type', () => {
  ...
  it('should have unique event_ids across all iOS events', () => {
    const ids = normalizedEvents.map((e) => e.event_id);
    expect(new Set(ids).size).toBe(ids.length);
  });

  it('should have unique idempotency_keys across all iOS events', () => {
  ...
  it('should have consistent consent status across all iOS events', () => {
    for (const event of normalizedEvents) {
  ...
  it('should have consistent tenant fields across all iOS events', () => {
    for (const event of normalizedEvents) {
  ...
  it('should have iOS-specific platform and SDK fields after normalization', () => {
    for (const event of normalizedEvents) {
      expect(event.platform_id).toBe('ios');
      expect(event.source.platform).toBe('ios');
      expect(event.source.sdk).toBe('@aether/ios');
      expect(event.source.environment).toBe('staging');
      expect(event.source.surface).toBe('ios_app');
      expect(event.sdk.name).toBe('@aether/ios');
      expect(event.sdk.version).toBe('0.1.0-alpha.0');
      expect(event.device.platform).toBe('ios');
    }
  });
  ...
  it('should preserve anonymous_id across all iOS events', () => {
    for (const event of normalizedEvents) {
  ...
  it('should have monotonically increasing timestamps across all iOS events', () => {
  ...
  it('should validate every iOS event has a complete EventEnvelope shape', () => {
  ...
  it('should assert iOS produces the same identity transition as all other platforms', () => {
  ...
  it('should assert iOS uses the same idempotency key format as all other platforms', () => {
    const keys = normalizedEvents.map((e) => e.idempotency_key);
    expect(keys).toEqual(
  ...
  it('should assert iOS produces the same schema hash as all other platforms', () => {
    expect(schema.schema_hash).toBe('sha256:canonical-first-value-journey-v1-2024-09-11');
  });

  it('should assert iOS shares the same tenant/source/app/surface fields as all other platforms', () => {
    ...
    expect(firstEvent.source.platform).toBe('ios');
    expect(firstEvent.source.surface).toBe('ios_app');
  });
});
