 ...
 * Android First-Value Journey Snapshot Test
 *
 * Blueprint §3.4 — Android emits the canonical-first-value-journey fixture through
 * Android-native/platform-idiomatic APIs and normalizes into the same canonical envelope.
 *
...
const PLATFORM_OVERRIDES = {
  platform_id: 'android',
  sdk_name: '@aether/android',
  device_platform: 'android',
  surface: 'android_app',
};

describe('PR D: Cross-Platform Parity — Android First-Value Journey', () => {
  ...
  // Normalize all events with Android-specific overrides
  ...
  it('should map each Android event to the correct step event_type', () => {
  ...
  it('should have unique event_ids across all Android events', () => {
    const ids = normalizedEvents.map((e) => e.event_id);
    expect(new Set(ids).size).toBe(ids.length);
  });

  it('should have unique idempotency_keys across all Android events', () => {
  ...
  it('should have consistent consent status across all Android events', () => {
    for (const event of normalizedEvents) {
  ...
  it('should have consistent tenant fields across all Android events', () => {
    for (const event of normalizedEvents) {
  ...
  it('should have Android-specific platform and SDK fields after normalization', () => {
    for (const event of normalizedEvents) {
      expect(event.platform_id).toBe('android');
      expect(event.source.platform).toBe('android');
      expect(event.source.sdk).toBe('@aether/android');
      expect(event.source.environment).toBe('staging');
      expect(event.source.surface).toBe('android_app');
      expect(event.sdk.name).toBe('@aether/android');
      expect(event.sdk.version).toBe('0.1.0-alpha.0');
      expect(event.device.platform).toBe('android');
    }
  });
  ...
  it('should preserve anonymous_id across all Android events', () => {
    for (const event of normalizedEvents) {
  ...
  it('should have monotonically increasing timestamps across all Android events', () => {
  ...
  it('should validate every Android event has a complete EventEnvelope shape', () => {
  ...
  it('should assert Android produces the same identity transition as all other platforms', () => {
  ...
  it('should assert Android uses the same idempotency key format as all other platforms', () => {
    const keys = normalizedEvents.map((e) => e.idempotency_key);
    expect(keys).toEqual(
  ...
  it('should assert Android produces the same schema hash as all other platforms', () => {
    expect(schema.schema_hash).toBe('sha256:canonical-first-value-journey-v1-2024-09-11');
  });

  it('should assert Android shares the same tenant/source/app/surface fields as all other platforms', () => {
    ...
    expect(firstEvent.source.platform).toBe('android');
    expect(firstEvent.source.surface).toBe('android_app');
  });
});
