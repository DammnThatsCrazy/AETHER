...
async def test_provider_observation_ingests_bronze_promotes_silver_and_persists_observation():
    ...
    fact = await pipeline.ingest_provider_observation(ProviderObservation(
        tenant_id="tenant-a", provider="rpc", source_record_id="log-1", source_execution_id="exec-1", source_manifest_id="manifest-1",
        observed_at="2026-07-05T00:00:00Z", chain_id="8453", network="base-mainnet", contract_or_mint="0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913",
        transaction_hash="0xabc", log_or_instruction_index=3, amount_atomic=2500000, from_address="0xpayer", to_address="0xmerchant", event_type=StablecoinEventType.PAYMENT, finality_status=FinalityState.CONFIRMED,
    ))
...
async def test_unknown_deployment_is_rejected_before_silver_or_graph():
    pipeline = StablecoinIngestionPipeline(bronze=BronzeRepository("stablecoin_pr2_unknown"), silver=SilverRepository("stablecoin_pr2_unknown"), observations=StablecoinObservationRepository())
    with pytest.raises(ValueError, match="unknown stablecoin deployment"):
        await pipeline.ingest_provider_observation(ProviderObservation(
            tenant_id="tenant-a", provider="rpc", source_record_id="log-1", source_execution_id="exec-1", source_manifest_id="manifest-1",
            observed_at="2026-07-05T00:00:00Z", chain_id="8453", network="base-mainnet", contract_or_mint="0x0000000000000000000000000000000000000000",
            transaction_hash="0xabc", amount_atomic=1,
        ))
...
async def test_finality_reorg_marks_downstream_correction_required():
    ...
    transition = await StablecoinFinalityService(repo).transition(obs_id, FinalityState.REVERTED, reason="reorg depth 4")
...
async def test_reconciliation_matches_partial_pending_and_reverted_states():
    ...
    matched = await service.reconcile(intent, OnchainEvidence("0x1", "0xpayer", "0xmerchant", "usdc:base", "8453", 100, FinalityState.FINALIZED))
    partial = await service.reconcile(intent, OnchainEvidence("0x2", "0xpayer", "0xmerchant", "usdc:base", "8453", 99, FinalityState.FINALIZED))
    pending = await service.reconcile(intent, OnchainEvidence("0x3", "0xpayer", "0xmerchant", "usdc:base", "8453", 100, FinalityState.CONFIRMED))
    reverted = await service.reconcile(intent, OnchainEvidence("0x4", "0xpayer", "0xmerchant", "usdc:base", "8453", 100, FinalityState.REVERTED))
...
def test_gold_materializer_excludes_pending_reverted_mints_and_internal_transfers():
    ...
    metrics = StablecoinGoldMaterializer().summarize_finalized_payment_volume(rows, window_start="2026-07-05T00:00:00Z", window_end="2026-07-06T00:00:00Z")
...
async def test_support_state_requires_evidence_and_blocks_invalid_transition():
    ...
    with pytest.raises(ValueError, match="evidence_reference"):
        await service.assert_support(SupportEvidence("t", "merchant", "usdc:base", StablecoinCapability.ACCEPT_PAYMENT, SupportState.ANNOUNCED, "public_claim", ""))
    record = await service.assert_support(SupportEvidence("t", "merchant", "usdc:base", StablecoinCapability.ACCEPT_PAYMENT, SupportState.ANNOUNCED, "public_claim", "url://claim"))
    assert record["support_state"] == "announced"
    with pytest.raises(ValueError, match="invalid support transition"):
        await service.assert_support(SupportEvidence("t", "merchant", "usdc:base", StablecoinCapability.ACCEPT_PAYMENT, SupportState.PRODUCTION_ACTIVE, "operator", "ticket://1"))
