...
async def test_kyber_operations_health_lineage_and_audited_remediation_are_tenant_scoped():
    ...
    await obs_repo.insert("obs-1", {"observation_id": "obs-1", "tenant_id": "tenant-a", "source": "rpc", "source_record_id": "log-1", "evidence_id": "bronze-1", "finality_status": "finalized", "event_type": "payment"})
    ...
    assert [step["layer"] for step in lineage["lineage"]] == ["provider", "bronze", "silver", "classification", "reconciliation", "gold", "graph", "profile360", "tenant_ui"]
