...
def test_missing_secret_scan_fails_closed():
    """A scan that did not run must FAIL the security row — never WARN."""
    ev = dict(gate.complete_evidence())
    ev["secret_findings"] = []
    ev["secret_scan_ran"] = False
    row = _row(gate.evaluate_matrix(ev), "secret_safe")
    assert row["status"] == "fail"
    assert "did not run" in row["detail"]
