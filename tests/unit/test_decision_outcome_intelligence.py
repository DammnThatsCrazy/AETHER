...
def test_kyber_strategic_observability_scores_and_aggregates():
    from services.admin.kyber_strategic import KyberStrategicObservability

    tenants = [{"id": "tenant-a", "name": "Tenant A", "plan": "enterprise"}, {"id": "tenant-b", "name": "Tenant B", "plan": "pro"}]
    recommendations = [
        {"recommendation_id": "rec-a", "tenant_id": "tenant-a", "recommendation_type": "fraud_review", "expected_value": 1000, "status": "decided", "confidence": {"overall": 0.8}, "created_at": "2026-05-30T00:00:00+00:00"},
        {"recommendation_id": "rec-b", "tenant_id": "tenant-b", "recommendation_type": "retention", "expected_value": 500, "status": "generated", "confidence": {"overall": 0.3, "freshness_penalty": 0.2}, "created_at": "2026-01-01T00:00:00+00:00"},
    ]
    decisions = [{"decision_id": "dec-a", "tenant_id": "tenant-a", "recommendation_id": "rec-a", "decision_status": "approved", "created_at": "2026-05-30T01:00:00+00:00"}]
    actions = [{"action_id": "act-a", "tenant_id": "tenant-a", "decision_id": "dec-a", "created_at": "2026-05-30T02:00:00+00:00"}]
    outcomes = [{"outcome_id": "out-a", "tenant_id": "tenant-a", "recommendation_id": "rec-a", "action_id": "act-a", "label": "success", "value": 750, "created_at": "2026-05-31T00:00:00+00:00"}]
    feedback = [{"tenant_id": "tenant-a", "recommendation_id": "rec-a", "confidence_delta": 0.05, "created_at": "2026-05-31T00:00:00+00:00"}]
    playbooks = [{"playbook_id": "pb-a", "tenant_id": "tenant-a", "template_id": "fraud_cluster_review", "category": "fraud_review", "created_at": "2026-05-29T00:00:00+00:00"}]
    runs = [{"run_id": "run-a", "tenant_id": "tenant-a", "playbook_id": "pb-a", "status": "completed", "generated_recommendation_ids": ["rec-a"], "outcome_ids": ["out-a"], "created_at": "2026-05-30T00:00:00+00:00"}]

    obs = KyberStrategicObservability(tenants, recommendations, decisions, actions, outcomes, feedback, playbooks, runs, "lifetime")
    overview = obs.strategic_overview()
    assert overview.total_tenants == 2
    assert overview.recommendations_generated == 2
    assert overview.outcomes_observed == 1
    assert overview.observed_value_total == 750
    assert overview.top_recommendation_family in {"fraud_review", "retention"}

    health = {item.tenant_id: item for item in obs.tenant_health()}
    assert health["tenant-a"].tenant_health_score > health["tenant-b"].tenant_health_score
    assert health["tenant-b"].churn_risk_score > health["tenant-a"].churn_risk_score

    families = {item.recommendation_family: item for item in obs.family_performance()}
    assert families["fraud_review"].approved == 1
    assert families["fraud_review"].observed_value == 750

    playbook_rows = obs.playbook_performance()
    assert playbook_rows[0].tenant_adoption_count == 1
    assert playbook_rows[0].observed_value == 750

    drift = obs.model_confidence_drift()
    assert drift.confidence_average == 0.55
    assert drift.low_confidence_rate == 0.5

    signals = obs.vertical_solution_signals()
    assert any(signal.solution_key == "fraud_risk" for signal in signals)
    opportunities = obs.revenue_opportunities()
    assert opportunities


def test_kyber_empty_state_is_safe_and_aggregate_only():
    from services.admin.kyber_strategic import KyberStrategicObservability

    obs = KyberStrategicObservability([], [], [], [], [], [], [], [], "30d")
    response = obs.response({"overview": obs.strategic_overview().model_dump()})
    assert response["data_freshness"]["status"] == "empty"
    assert response["overview"]["total_tenants"] == 0
    assert obs.family_performance() == []
    assert obs.playbook_performance() == []
    assert obs.revenue_opportunities() == []


def test_action_target_registry_descriptors_and_simulated_receipts():
    import asyncio

    from services.intelligence.action_targets import ActionTargetRegistry
    from services.intelligence.decision_models import ActionDispatch, ActionIntegrationConfig

    registry = ActionTargetRegistry()
    target_types = {target.target_type for target in registry.targets}
    assert target_types == {"slack", "webhook", "crm_task", "marketing_automation", "ticketing", "agent_assist"}
    webhook = registry.require("webhook")
    descriptor = webhook.descriptor()
    assert descriptor.supports_delivery_receipts is True
    config = ActionIntegrationConfig(
        ...
        default_destination="https://example.test/webhook",
    ...
    receipt = asyncio.run(webhook.dispatch(dispatch, config))
    assert receipt.dispatch_id == "disp-1"
    assert receipt.delivered_at
    assert receipt.external_url == "https://example.test/webhook"
