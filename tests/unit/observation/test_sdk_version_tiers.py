...
def test_classify_version_parsing_variants():
    ...
    assert st.classify_sdk_version("v0.1.0").id == "supported"
    assert st.classify_sdk_version("0.1.0-rc.1").id == "supported"
    assert st.classify_sdk_version("0.1.0+build.7").id == "supported"
    assert st.classify_sdk_version("0.1").id == "supported"  # patch defaults to 0
    ...
    assert st.classify_sdk_version("0").id == "unclassified"
    assert st.classify_sdk_version("0.x").id == "unclassified"
...
def test_classify_version_name_recognition_is_advisory():
    ...
    assert st.classify_sdk_version("0.1.0", "analytics.js").id == "supported"
...
def test_supported_band_carries_full_capabilities():
    sup = st.classify_sdk_version("0.1.0-alpha.0")
    assert st.CAP_ENVELOPE_B in sup.capabilities
    assert st.CAP_NORMALIZATION_SPINE in sup.capabilities
    assert st.CAP_BATCH_INGESTION in sup.capabilities
    assert st.CAP_SERVER_SIDE in sup.capabilities
    assert st.CAP_REPLAY in sup.capabilities

    unsup = st.classify_sdk_version("0.0.9")
    assert unsup.blocked_after == st.BLOCKED_AFTER_DATE
...
def test_advisory_is_none_when_flag_off(monkeypatch):
    monkeypatch.setattr(st, "settings", _settings(enabled=False))
    assert st.sdk_version_advisory({"name": "@aether/web", "version": "0.1.0-alpha.0"}) is None
...
def test_advisory_labels_tier_and_capabilities(monkeypatch):
    monkeypatch.setattr(st, "settings", _settings(enabled=True, mode="shadow"))
    adv = st.sdk_version_advisory({"name": "@aether/web", "version": "0.1.0-alpha.0"})
    ...
    unsup = st.sdk_version_advisory({"name": "@aether/web", "version": "0.0.9"})
    assert unsup["tier"] == "unsupported"
    assert unsup["blocked_after"] == st.BLOCKED_AFTER_DATE
...
def test_blocked_never_blocks_supported_or_unclassified(monkeypatch):
    ...
    assert st.sdk_version_ingress_blocked(
        {"name": "@aether/web", "version": "0.1.0-alpha.0"}
    ) is False
    assert st.sdk_version_ingress_blocked(
        {"name": "@aether/web", "version": "0.2.0"}
    ) is False
