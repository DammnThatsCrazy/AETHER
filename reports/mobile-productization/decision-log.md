...
- **Reuse statement:**
  ...
  - *New boundary:* `services/backend/services/continuation/store.py` with direct SQL + a real migration, mirroring
...
- **Reuse statement:**
  - *Existing system considered:* `services/backend/services/realtime/` monotonic channel cursors.
  ...
  - *New boundary:* `services/backend/services/client_sync/` — durable `sync_change_log` + `sync_cursor_counter`
...
- **Comms branch parked:** `claude/aether-comms-multi-provider` (ADR-C11 comms cohort) is out of
  scope — separate observe-external-ESP domain (`services/backend/services/comms/`); its 12 unmerged commits stay
  untouched. No completion commit touches `services/backend/services/comms/`.

## D11 — Mobile notification projection = redacted push, no raw payload
- **Reuse statement:** the delivery adapters (`services/backend/services/delivery/adapters/_notification_base.py`)
...
- **Gateway projections (M3) reuse owning services** (`services/backend/services/profile/*`, `services/backend/services/campaign/*`,
  `notification_intelligence/inbox`, `services/backend/services/exploration/store`, `noesis`) — bounded, redacted,
  data-truth-preserving. NEVER re-calculate Profile360/Campaign360/graph truth.
- **Mobile action adapter (M6) reuses `services/backend/services/kyber/ops/*`** — no second command plane, no generic
...
- **MobileNotificationProjection (M1a):** existing — `_push_alert()` (redacted only body, and only via a `redacted_body` key no builder ever set → every real push shipped the generic fallback; shipped RAW title; had an `allow_full_content` raw-body push path) + `notification_inbox` canonical store. None satisfy the D11 redacted-push requirement. New boundary: `services/backend/services/notification_intelligence/projection.py` derives a redacted, bounded push surface (amounts/PII → `[redacted]`, truncation limits) at creation (inbox) or the push boundary, and is the ONLY content push adapters consume; deep-link class reuses continuation-plane surfaces. Validators: adapters fail closed with no projection AND no source content; `allow_full_content` no longer ships raw body on push; parity tests force the canonical event to carry the 5 `push_*` fields (no second record).
- **Delivery-safety validator (M1b):** existing — `check_delivery_topology.py` (topology/config contract, not code patterns), `check_dsr_coverage.py` (DSR reachability), `_common.py` (Reporter/main_guard/repo_root — REUSED). None detect the 5 unsafe code patterns. New boundary: AST-only scan of `services/backend/services/delivery/**` + `services/backend/services/notification_intelligence/**`; stdlib-only; pure `analyze()` for testability; wired into `make ci-check` via repo_doctor + `delivery-safety-check` target. Validator: the gate fails the build if the 5 patterns recur; 20 unit tests pin accept/reject behavior.

#### D12 appendix — M3 reuse statements (landed)
- **Gateway projections (M3a):** existing — `Profile360Aggregator.summary` (services/backend/services/profile), `CampaignRepository` + `CampaignPopulationExplorer` (services/backend/services/campaign), the SINGLE canonical inbox (`notification_intelligence.inbox`), `ExplorationViewRepository` (services/backend/services/exploration/store), `NoesisConversationStore` (services/backend/services/noesis), and the D11 projection helper (`services/backend/services/notification_intelligence/projection.py` — `_redact`/`_truncate`/`build_projection` REUSED, never duplicated). None expose a bounded, redacted, mobile-sized view. New boundary: `services/backend/services/mobile/projections.py` (`MobileProjectionService` + builders) composes owning-service truth — bounded, redacted, data-truth-preserving — exposed via `GET /v1/mobile/{today,profile,campaign,alerts,briefing}` on the existing mobile gateway. Validators: `test_mobile_projections.py` (22 tests, injected collaborators); no projection re-calculates Profile360/Campaign360/graph truth (imports are composition-only).
...
- **Demo-seed extensions for the six M7 domains (M7a):** existing — the demo-seed machinery
  (`services/backend/services/demo_seed/*`: `SeedRecord`/`stable_id`/`build_manifest`, `assert_seed_allowed`,
  ...
  invisible store. New boundary: `services/backend/services/demo_seed/repositories.py` adds
...
- **Mobile-actions tenant scope (M8-D3):** existing — the M6 mobile action digest
  (`services/backend/services/kyber/ops/mobile_actions.py`, `GET /v1/kyber/mobile/actions`) surfaced
