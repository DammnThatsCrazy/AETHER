// examples/web-next/src/sdk.ts — adapted from apps/proof-web/src/sdk.ts
...
/** Initialize the Aether SDK. Part of the first-value journey: install → init. */
export async function initSDK(
  ...
  const sdkConfig: AetherConfig = {
    ...
    endpoint,
    debug: true,
    application: { name: 'examples-web-next', version: SDK_VERSION },
...
/** First-value heartbeat — proves the SDK is live and the backend is reachable. */
export async function emitHeartbeat(): Promise<{
  ...
  aether.track('heartbeat', { eventId, source: 'examples-web-next' });

  return { eventId, status: 'queued' };
}

/** Page view — first contentful event after init. */
...
/** Custom event — general-purpose behavioral signal. */
...
/** Identify user — attaches known identity to the anonymous session. */
export async function identifyUser(
  ...
  // Prepare the hydration payload.
  aether.hydrateIdentity({ userId });

  return { anonymousId, userId, status: 'queued' };
}

/** Commerce event — revenue-bearing conversion for the first-value journey. */
...
/** Simulate offline — queues events for retry, proves flush recovery. */
export async function simulateOffline(): Promise<{
  ...
  aether.track('offline_mode_entered', {
    wasOnline,
    source: 'examples-web-next',
  });
...
/** Reset session — clears identity and queue for a fresh first-value journey. */
...
/** Raw consent state from the @aether/web SDK. */
export function getRealConsentState(): ConsentState | null {
  return tracking.consentState;
}

/** Live SDK instance reference, null before init. */
