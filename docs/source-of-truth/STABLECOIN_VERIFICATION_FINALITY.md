...
`StablecoinSolanaTransactionVerifier` reads an existing observation, enforces tenant scope, resolves an SPL-token deployment from the registry, fetches `sol_getTransaction`, and verifies SPL mint evidence from token balances or parsed token instructions before finality changes are allowed. Missing transactions move observations to `pending`, failed transactions move eligible observations to `failed` or `reverted`, and successful transactions advance to `confirmed` or `finalized` based on a configurable slot threshold.
...
This layer is not a live polling scheduler and does not execute payments. Live provider polling, explorer/RPC disagreement comparison, production replay queues, and staging provider evidence remain separate rollout requirements.
