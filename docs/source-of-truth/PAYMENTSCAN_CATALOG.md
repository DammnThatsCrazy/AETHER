...
PaymentScan-derived V1 records remain catalog or benchmark-only unless another tenant-authorized source supplies user-level evidence. Kyber diagnostics expose PaymentScan as `catalog_and_benchmarks_only`, and graph/Profile360/Campaign360 rollups display basis breakdowns so PaymentScan benchmark records cannot be mistaken for deterministic card spend.
