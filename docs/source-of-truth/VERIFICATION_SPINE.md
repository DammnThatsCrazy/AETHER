...
Hard-budget control-plane commands are launched in isolated process groups.
On timeout, the runner terminates the group before recording failure, so
pytest descendants cannot survive a timed-out wrapper and affect a later
verification or staging-related control run.
