...
- The classifier is a **pure, server-side function**
  (`services/backend/services/traffic/classifier.py`, `SourceClassifier.classify`). SDKs submit
...
`redirect_verified_source_link` (`services/backend/services/traffic/routes.py`) reads
...
(`_clean_destination_url` in `services/backend/services/traffic/referral_links.py`). The only
...
(`services/backend/services/traffic/repair.py` preserves the existing hash rather than
...
campaign" (`services/backend/services/attribution/apple_postbacks.py` module docstring).
...
(`services/backend/services/traffic/routes.py`) calls the server classifier and then **overwrites**
...
- The reclassification repair "never overwrites classification history"
  (`services/backend/services/traffic/repair.py` module docstring); it records new revisions with
...
(`services/backend/services/attribution/apple_postbacks.py`, `_build_signed_message` /
