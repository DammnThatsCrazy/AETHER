...
Streaming variant: `POST /v1/noesis/query/stream` returns server-sent events with `status`, `answer`, and `final` events. The `final` event is the same normalized Noesis response payload.
...
Conversation records store user/assistant turns, a redacted response payload, surface, tenant scope, title, and timestamps. Stored assistant payloads intentionally omit `query_debug`. Export and delete endpoints use the same tenant/RBAC scope checks as history reads.
...
- Define automatic retention windows for Noesis conversations once product and compliance requirements are finalized. Manual tenant/operator-scoped export and deletion controls are implemented.
