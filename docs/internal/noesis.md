...
Conversation records store user/assistant turns, a redacted response payload, surface, tenant scope, title, and timestamps. Stored assistant payloads intentionally omit `query_debug`. Conversation ids are scoped to their original tenant and surface before appending follow-up turns, preventing a caller from reusing another tenant's conversation id. Export and delete endpoints use the same tenant/RBAC scope checks as history reads. Purge is operator/admin-only and can apply a retention window by surface or across Noesis history.
