...
Kyber now consumes aggregate Decision & Outcome Intelligence records through admin-only strategic observability endpoints. These views show OODA health, model confidence health, tenant value, and revenue opportunities without exposing raw tenant-private graph intelligence across tenants. See [Kyber Strategic Observability](./KYBER-STRATEGIC-OBSERVABILITY.md).

## Integration actions and dispatch

Approved OODA decisions can now be dispatched into governed integration targets through Action Dispatch. Dispatch validates tenant isolation, approved decisions, recommendation matching, elevated/critical approval metadata, idempotency, delivery receipts, lifecycle events, and revenue metering records. See [Integration Actions](./INTEGRATION-ACTIONS.md) and [Action Dispatch](./ACTION-DISPATCH.md).
