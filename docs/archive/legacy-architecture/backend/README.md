...
This directory is a historical archive of the former backend root. The
canonical implementation is now the Python monolith under
`services/backend/`; this archive is not a runtime input and must not be
extended. The following material was retained here because it was formerly
co-located with the backend but is un-deployed, un-wired, superseded, or
historical:
...
work into the canonical `services/backend/` tree. Registration status and the full
gap ledger are tracked in
`docs/productization/sdk-universal-ingestion-alignment/REPO_TRUTH_AND_GAP_MATRIX.md`.
The archive is intentionally retained for traceability and is guarded by the
repository-tree ownership validator.
