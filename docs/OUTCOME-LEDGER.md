...
Kyber uses tenant-scoped Outcome Ledger outputs and OODA repositories to compute internal tenant value health, expansion readiness, churn risk, and revenue opportunities. Cross-tenant Kyber views are aggregate or tenant-account diagnostics only and must not include raw tenant graph intelligence. See [Kyber Revenue Intelligence](./KYBER-REVENUE-INTELLIGENCE.md).
