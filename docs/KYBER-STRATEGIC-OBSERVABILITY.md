...
GET /v1/admin/kyber/integration-health?window=30d
...
Kyber aggregates integration health from action dispatches, delivery receipts, integration configs, and revenue metering events. Operators can inspect dispatch volume by tenant, success/failure rates, retry rates, premium connector usage, manual-vs-integrated action ratio, stale approved actions, action-to-outcome conversion, and integration upsell opportunities without exposing raw tenant-private graph intelligence. See [Revenue Metering Events](./REVENUE-METERING-EVENTS.md).
