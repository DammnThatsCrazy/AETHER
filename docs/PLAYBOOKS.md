...
## Kyber playbook performance

Kyber aggregates playbook template adoption, run completion, stale/incomplete runs, observed value, pending value, and recommended packaging for Olympus operators. These are admin-only aggregate diagnostics, not tenant-facing cross-tenant data. See [Kyber Strategic Observability](./KYBER-STRATEGIC-OBSERVABILITY.md).

## Playbooks and integration dispatch

Playbook-generated recommendations can flow into the same governed action dispatch lifecycle after human approval. Dispatched actions remain linked to recommendations, playbook runs, delivery receipts, outcomes, Outcome Ledger value, Playbook ROI, and revenue metering events. See [Action Dispatch](./ACTION-DISPATCH.md).
