...
end-to-end-wired subsystems — the provider-neutral `services/backend/services/comms` pipeline
(connector → `CommunicationEventPayload` → silver dispatcher →
`silver_comms_facts` → campaign resolver → measurement → attribution → graph
projection → state), the `services/backend/services/agent_comm_observability` agent-mail
observability layer, the observation-only `services/backend/services/agentic_observability`
...
| "Contract Spine integration (§29): 24 required communication contract modules" | Declarative registries in `packages/shared/contracts/*.json` → generated Python/TS twins via `scripts/generate_platform_contracts.py`. Many of the blueprint's objects already exist in shipped form (`services/backend/services/comms`, `services/backend/services/delegation`, `services/backend/services/identity`, agentic observability, OI `EvidenceRef`/projection `ClaimEnvelope`); a smaller set is genuinely new and must be registered without re-declaring canonical primitives |
...
| "Sender is not author; principal/delegation (§4.2, §51–52)" | Partial. `services/backend/services/comms` carries `ActorKind`/`Direction`/roles; `services/backend/services/identity` (`EntityType`) and `services/backend/services/delegation` (`DelegationGrant`) ship; but there is no temporally-valid full role matrix (author/generator/editor/approver/sender/presented_sender/principal/delegator/beneficiary/accountable_party) over a single canonical communication |
...
| "InformationTransfer is the parent abstraction (§5.3, §6)" | No `InformationTransfer` object; today agent-runtime transfers are approximated by agentic `agent.*` events and `services/backend/services/agent_comm_observability` records, never as a single transfer taxonomy |
...
| "Harness hooks + event taxonomy (§37–38)" | The dotted event taxonomy maps onto the repo's `family` + snake_case `type` model in `packages/shared/contracts/event-registry.json` (`comms`, `agent`, `journey`, … families) and the observation-only SDK envelope (`execution_by_aether` is always `false`). Harness observability hooks largely exist across `services/backend/services/agentic_observability` + `services/backend/services/agent_comm_observability`; mapping, not building, is the work |
...
| Blueprint / DoD docs | `services/backend/services/comms` + ADR-C1…C11 architecture docs live under `docs/comms/`; no Communication360 program doc exists | **No architecture SoT reconciling the master blueprint to ADR-010 + registries; no vertical-slice blueprint.** This program's Phase 1 lands both |
...
identity resolution (`services/backend/services/identity`), `services/backend/services/delegation`
(`DelegationGrant`, scoped/time-bound/revocable), the comms silver path
(`services/backend/services/comms` → `silver_comms_facts`, campaign resolver, measurement silver
...
| 2026-09-03 | 3 | SHIPPED — canonical comms contracts + storage + registries. Contract-surface freeze (`ce2a412e`) ratifies the Phase-2 canonical model into `services/backend/services/communication360/contracts.py` (conversation/provider-thread/matter, communication act/request/commitment/response-expectation, information + claim bindings + transformations, knowledge/interpretation/context-inclusion, participant/principal bindings) — every fact `claim_state`-capped `observed`, never above; then typed storage (`f6a78434`): `communication360_facts` JSONB store (`repository.py`, tenant-scoped, idempotent upsert, in-memory fallback, `run_id`/`context_hash` columns referencing the `computation_runs` seam, `source_event_id`/`source_event_type` onto existing comms events) + Alembic `20260903_communication360_canonical_facts`; seeded dimension vocabularies (`registry.py`); `AETHER_COMMUNICATION360_ENABLED` flag default OFF (`Communication360Config` added to `config/settings.py`, additive only). Contract/registry/repository suites green; projection row stays `in_flight` |
| 2026-09-03 | 4 | SHIPPED — provider + projection convergence (core wiring). `Communication360Provider` (`services/backend/services/communication360/provider.py`, `projection_id: communication360`, read-only) implementing `IntelligenceProjectionProvider` with typed `SilverCommsSource` + `CanonicalFactSource` read-overs (`reader.py`) — an unreachable backing source degrades the affected section to a typed `degraded`/`missing` state (never crashes, never fabricates numbers); `register_provider` registration seam exported through the barrel. Merge `d3828790` (ws-p4 `8dcb0991`). Per the infra360 template, NO bespoke surface-capability row or exploration adapter was added (the row's `surfaceIds` `timeline`/`profile360` already exist) and the router is NOT mounted in `main.py` (plane wiring is a central concern; `AETHER_COMMUNICATION360_ENABLED` stays OFF); provider + reader suites green |
| 2026-09-03 | 5 | SHIPPED — information fidelity + knowledge/interpretation + authority. `fidelity.py` computes the seven §71 information-fidelity metrics over `InformationTransformation` lineage (claim/citation/evidence retention, `semantic_drift`, `omission_rate`, `unsupported_addition_rate`, `contradiction_rate`) with `FidelityReport.claim_state` capped `observed` + confidence = measured-hop coverage; `knowledge.py` maps knowledge/interpretation/context-inclusion records (R4, observed-capped); `authority.py` evaluates delegation-outcome → authority-state purely (consumes only the engine's `allowed` outcome — never a silent grant; explicit `partial`/`degraded` for scope- or amount-limited grants), mirroring an observed delegation outcome so `claim_state` stays `observed`. Merge `2176861c` (ws-p5 `daf8bd09`). Central metric registration (`4cd1f073`): the seven produced metrics added to `metric-registry.json` + `shared/measurement/registry.py` with a field-for-field parity test (registry 20→27); §71's `constraint_retention_rate` has no producer and was deliberately NOT registered (gap recorded in the parity-test docstring, decision #7). Fidelity/knowledge/authority + parity suites green |
| 2026-09-03 | 6 | SHIPPED — resolution + downstream read surface + flip. Resolution engine (`1ac43940` via merge `a93430ae`): conversation/matter resolution over declared lineage only — causal order never inferred from timestamps; read-only `/v1/communication360` route surface + tests (`5ca44ef9`), mirroring the infra360 template exactly: `GET /{subject_kind}/{subject_id}` + `GET /health`, tenant-scoped, `read` + `communication360.read` capability gate fail-closed on undeclared capability, `PROJECTION_ID`/`READ_CAPABILITY`/`EXPLORE_CAPABILITY` constants, route classified in `config/route_registry.yaml`, NO `main.py` mount (flag-gated OFF). Registry flip: `communication360` → `implementationState: implemented`, `legacyBindings.migrationMode: converged`, `metricRefs` + the seven fidelity metrics, `canonicalAuthorities` (`communication_facts`/`campaign_touchpoints`/`entities`/`outcomes`/`evidence`) all resolving in the platform `AUTHORITY_INDEX` — no authority/spine-INDEX change needed (R5); `ownsCanonicalTruth` false + `graphMutationPolicy` read_only + zero pending unchanged. Twins regenerated (still 19 projections), projection validator clean. Package docstring enumerates all modules (`d8a08b92`); REPO-INDEX synced (`15764a60`); the 8 stale source-linked docs reviewed against `source_files` and ARCHITECTURE/BACKEND-API content updated for the flip + `/v1/communication360` surface (`fbdffe13`). Full-lane targeted suite 228 passed. Release gate deferred to the post-merge re-cut (decision #8); canonical `make ci-check` is the final gate |

When a phase lands, its row is updated here; the phase map in §2 carries the
current status. As of this row Phases 1–6 have shipped; the vertical-slice flip
landed in-lane (decision #8), and the canonical release gate runs post-merge.
...
2. **Information layer home.** The new `Information`/`InformationRef`/
   ...
   **RESOLVED (Phase 2, 2026-09-03, R5):** the information-layer facts land under
   ...
   read authorities.
   **RATIFIED (Phase 6, 2026-09-03):** no new `AUTHORITY_INDEX` authority was
   added. The Phase-3 Alembic migration registers the `communication360_facts`
   JSONB store as an instance of the existing `communication_facts` read
   ...
   is unchanged — it names spine-plane rows only, and the projection adds none.
   `ownsCanonicalTruth` stays `false`.
3. **Edges.** Add domain `EdgeType`s only where a real graph gap exists, each
   ...
   **RESOLVED (Phase 6, 2026-09-03):** no domain `EdgeType` was added. The graph-gap
   candidates examined across Phases 3–6 have no write producer — the projection is
   `graphMutationPolicy: read_only` and never writes edges — so each was rejected
   (adding `EdgeType`/`relationship_layers` entries without a producing lane would
   fabricate a graph the slice does not own); the candidates stay deferred until a
   producer lane converges them. Bare lifecycle members remain prohibited.
4. **Events.** Map blueprint events onto the repo's `family`+`type`
   ...
   **RESOLVED (Phase 6, 2026-09-03):** no `event-registry` addition was made. The
   fact store rows carry `source_event_id`/`source_event_type` referencing the
   existing comms events the convergence reads over; the blueprint event vocabulary
   maps onto the repo `family`+`type` model and none required a new member for the
   read-only slice.
5. **Runs/replay.** Reuse `computation_runs` + `context_hash` and the comms
   ...
   **RESOLVED (Phase 3, 2026-09-03):** `communication360_facts` rows carry
   `run_id` + `context_hash` referencing the `computation_runs`/`context_hash`
   seam, with idempotent upsert and an in-memory fallback; no parallel run or sync
   table was added.
6. **Capability.** Provider capability truth-telling extends the connector
   ...
   **RESOLVED (Phase 4/6, 2026-09-03):** reader/provider truth-telling holds — an
   unreachable backing source degrades the affected section to a typed
   `degraded`/`missing` state and never invents zero counts (reader contract
   tests); the read surface is capability-gated on `communication360.read`, which
   is genuinely declared on the row (`capabilityKeys`) and enforced fail-closed —
   a gate for an undeclared capability is a hard denial.
7. **Metric registration.** Register metrics in the dual-source registry ONLY
   with their producers, so a registry entry never claims a metric nothing
   computes.
   **RESOLVED (Phase 5, 2026-09-03):** the seven §71 fidelity metrics produced by
   `fidelity.py` (claim/citation/evidence retention, `semantic_drift`,
   `omission_rate`, `unsupported_addition_rate`, `contradiction_rate`) were
   registered in `metric-registry.json` + `shared/measurement/registry.py` with a
   field-for-field parity test (registry 20→27); §71's `constraint_retention_rate`
   has no producer in the slice and was deliberately NOT registered — the gap is
   recorded in the parity-test docstring, not silently stamped.
8. **Flip cadence.** When to flip the `communication360` registry row to
   `implemented`/`converged` and when to claim release readiness.
   **RESOLVED (Phase 6, 2026-09-03):** the row flips in-lane (this program) with
   zero pending declarations, `ownsCanonicalTruth: false` and
   `graphMutationPolicy: read_only` unchanged, and `metricRefs` absorbed; the
   canonical `make release-gate` is deferred to the post-merge re-cut (risk-fraud
   precedent), so this lane claims `make ci-check` green only when it exits 0 and
   makes no `production_ready` claim.
