...
| T2.3 | Surface-capability block for `temporal360` (dedicated `temporal360` surface; sibling `timeline`/`temporal_observatory` stay with their owners) + regen; late-arrival recompute following the measurement pattern; DSR check on any new replay artifact | Surface routable on `/v1/explore/query`; recompute + erasure honest | SHIPPED — 5b33f520 |
