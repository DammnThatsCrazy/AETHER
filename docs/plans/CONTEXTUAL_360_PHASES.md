...
| Context capsule / ingestion geo | [CONTEXT_INTELLIGENCE.md](../../docs/source-of-truth/CONTEXT_INTELLIGENCE.md) + `context-capsule-registry.json` + `services/backend/services/ingestion/geo_provider.py` | Network-egress geo semantics, precision classes, retention; **source-linked** (drift-reviewed when its source files change) |
...
| Measurement / value | `shared/measurement/` + `services/backend/services/value/` + `packages/shared/value.ts` | `MeasurementResult`/`ValueState`/`MetricDefinition`; money + FX provenance (no domain FX) |
| Identity/resolution | `services/backend/services/identity/` | `IdentitySubject`, resolution/merge, canonical ids |
| Consent / DSR | `services/backend/services/consent/` + `services/backend/services/dsr_propagation/` | Fail-closed consent evaluation; the 26 `DSR_COMPONENTS` erasure propagation spine |
| Evidence | `EvidenceRef` (`services/backend/services/operational_intelligence/models.py`) + `ProvenanceEnvelope` | Evidence references, provenance |
...
| `SubjectRef` | `ProjectionSubject` — canonicalized in Phase 1 to a single exploration→projection join (`services/backend/services/exploration/projection_subject.projection_subject_for`) shared by the surface adapter and the fabric's session composition; ad-hoc provider `{"kind","id"}` copies removed. Cross-plane seams (`ComputationContext.subject_type`/`subject_id`, `/v1/infrastructure/{subject_kind}/{subject_id}` path params, Noesis read kwargs) translate at their own edge and are deliberately not folded in |
...
| §87–93 metrics / money | `shared/measurement` + `services/backend/services/value` + `packages/shared/value.ts` |
...
| **2** — `temporal360` (root of the cluster) | The `graph_history_replay` authority (system-time/knowledge-as-of reconstruction from `graph_fact_versions`, extending `services/backend/services/operational_intelligence` + `shared/graph/traversal.py`; materialize `replay_ledger` beyond digest); `temporal360` provider + surface-capability entry + lenses; late-arrival recompute following the measurement pattern; `docs/blueprints/temporal360.md`; registry → `implemented` | Phase 1 landed | `temporal360` answers reality-vs-knowledge as-of via the exploration surface; 13-gate checklist passes; gate green | SHIPPED 2026-09-03 — T2.1–T2.4 landed (replay authority 1cf91b56, provider ec513f60, surface+adapter e31f9724, registry `implemented` + boot wiring e5330973); ledger rows 2026-09-03 (Phase 2 T2.1–T2.4) carry the detail |
...
| **4** — `geographic360` | The **one new canonical location primitive**: `location-registry.json` + `shared/geo` + `services/backend/services/geo` (LocationFact with roles/precision/coordinates; Place/Region/Jurisdiction; region-type hierarchy — not US-only), H3 client-side cells, geodesy via pure-python libs, `GeocodingProvider` behind the existing credential vault; graph edges with new `EdgeType` + `relationship_layers` entry; jurisdiction-vs-location and privacy downgrade (`exact→city→metro`) with `suppressed`/`precision_reduced`; provider lenses; DSR components; `docs/blueprints/geographic360.md`; registry → `implemented` | Phase 2 landed (temporal dep); location authority design reviewed | geographic360 answers on the exploration surface with precision never exceeding evidence; parity/gate green | SHIPPED 2026-09-03 — G4.1–G4.5 landed (canonical location authority b3e06071, graph vocabulary 0a028490, spatial substrate 14dc8ea7, provider 55ccc490 + dedicated surface/adapter 306a80a9, canonical `location_facts` store + `context_capsule_semantics` + DSR eraser + `implemented` flip a2ebd71e/48394a10/134dea0d/b143e554); ledger rows 2026-09-03 (Phase 4 G4.1–G4.5) carry the detail; **no `production_ready` claim** |
| **5** — Cross-cutting close-out | Cross-dimensional composition (temporal × geographic × population via the engine composition pattern); cross-360 monetary metrics consume canonical `value.ts`/FX only; full gates + docs regen; PR | Phases 2–4 landed | All three `implemented`; `make ci-check` green; `make release-gate` only if readiness is claimed | SHIPPED 2026-09-04 — X5.1–X5.3 landed (context-triad cross-360 engine composition 81f1aad6; cross-360 monetary/FX canonical-seam guard bd00ace6; docs close-out below); ledger rows 2026-09-04 (Phase 5 X5.1–X5.3) carry the detail; exit gate `make ci-check` green; **no `production_ready` claim** |
...
| --- | --- | --- | --- |
| T2.1 | `graph_history_replay` authority — read-side knowledge-time reconstruction from the ledger prefix + `graph_fact_versions` (extending `services/backend/services/operational_intelligence` + `shared/graph/traversal.py`); `replay_ledger` beyond digest; digest-verifiable | Reconstruction at knowledge instant τ equals the ledger prefix closed at τ; `KNOWN_THEN` served; no write path | SHIPPED — 1cf91b56 |
...
| P3.3 | `DSR_COMPONENT` coverage for `populations`/memberships/snapshots in `services/backend/services/dsr_propagation/` | Erasure of a member recomputes counts honestly; component count grows past 26 | SHIPPED — 9fb7f9de |
...
| G4.2 | Graph vocabulary: new `EdgeType` + `relationship_layers` entry; `services/backend/services/geo` surfacing; precision classes aligned to the `coarse_cell` taxonomy | Location edges classified + evidence-carrying; unclassified edges still error in staging/prod | SHIPPED — 0a028490 |
...
| G4.4 | `geographic360` provider (`summary`/`state`/`timeline`/`evidence`/`findings`; `exact→city→metro` privacy downgrade) + dedicated `geographic360` surface-capability block + `Geographic360SurfaceAdapter` + lenses | Provider + surface routable on `/v1/explore/query`; precision never exceeds evidence | SHIPPED — 55ccc490, 306a80a9 |
| G4.5 | Canonical `location_facts` store + store-backed default reader (C1); `context_capsule_semantics` authority (C2); `DSR_COMPONENT` for location facts + geographic erasure plane (C3); blueprint review; zero-pending (`context_capsule_semantics` resolved into SPINE_INDEX); source-linked docs stamped after review (C5) | 13-gate checklist passes; `make ci-check` green; row → `implemented` | SHIPPED — C1 a2ebd71e, C2 48394a10, C3 134dea0d, C4 b143e554 (docs close-out after C5) |

#### Phase 5 → cross-cutting close-out (no vertical slice; spec = §3 row 5 + §4 rules)

| WS | What ships | Exit criteria | Status |
| --- | --- | --- | --- |
| X5.1 | Context-triad cross-360 composition: `compose_context_triad` in `shared/projection_engine/composition.py` (geographic360 × temporal360 × population360 over one tenant-scoped subject via the S6 engine-composition pattern — the family rule, never a `Spatiotemporal360`) | Composition of the three projections answers on the engine; per-subject-kind typed drops; no new projection/backend | SHIPPED — 81f1aad6 |
| X5.2 | Cross-360 monetary/FX canonical-seam guard: `scripts/validate_cross360_monetary_fx.py` (context leaves + `services/backend/services/geo` + exploration path + composition seam stay monetary-free) + repo-doctor registration + `FINANCIAL_VALUE_SEMANTICS.md` rule 8 (one FX seam — no geography- or population-specific FX) | A new non-canonical monetary/FX occurrence fails CI; allowlist empty (shrink-only) | SHIPPED — bd00ace6 |
| X5.3 | Docs close-out: `PROJECTION_ENGINE_ARCHITECTURE.md` S6 reviewed (five entry points + context family); §3 phase rows + §3.1 tables + §6 ledger; full gates | All three `implemented`; `make ci-check` green | SHIPPED — this row + ledger |
...
before the row flips. Phase 5 has no blueprint (it is cross-cutting close-out);
its composition workstream extends the S6 engine composition the plan row names.
...
7. **Erasure is not a dead end.** Any persisted artifact a phase introduces
   ...
   `DSR_COMPONENT` in `services/backend/services/dsr_propagation/` in the same phase.
...
- `services/backend/services/infrastructure` is never imported/mounted in `main.py`, so the
...
(`services/backend/dependencies/projection_plane.py`) — the
...
idempotent across repeated boot entries); `services/backend/services/infrastructure` is imported
...
`services/backend/tests/unit/test_projection_plane_boot_wiring.py`).
...
- System-time ("as known at knowledge time") reconstruction from
  ...
  builds.
- Do-not-duplicate boundaries: `shared/temporal/` (kernel), `services/backend/services/temporal_preferences/`
...
- Membership is **never** written as a graph `MEMBER_OF` edge (no gateway
  reference in `services/backend/services/population/`); only a `population_memberships` table row
...
- Edge provenance: temporal properties supported; **`membership_state`,
  ...
  property vocabulary.
- Consent: `services/backend/services/population/` has zero consent/policy references — membership
...
- Canonical authorities already declared for `geographic360`:
  ...
  `temporal`; surface `geo`; legacy binding `services/backend/services/geo`; blueprint
...
- **LocationFact home (A):** new `packages/shared/contracts/location-registry.json`
  + `services/backend/shared/geo/models.py`, surfaced through
  `services/backend/services/geo`. Coordinates must NOT be added to the context capsule (parity
...
| 2026-09-03 | Phase 4 (G4.2 — graph vocabulary + `services/backend/services/geo` surface) | The location-fact graph vocabulary ships (0a028490): VertexTypes `PLACE`/`REGION`/`JURISDICTION` + EdgeTypes `LOCATED_AT` (subject → Region) / `OBSERVED_IN` (subject → Place) / `UNDER_JURISDICTION` (subject → Jurisdiction), each mapped to `RelationshipLayer.EXCLUDED` in `relationship_layers._EDGE_LAYER_MAP` (Python-only, mirroring `SEMANTIC_RELATES_TO` — unclassified edges still error in staging/prod, TS parity surface untouched). `services/backend/services/geo/location_edges.py` is the ONE assembly surface (services/backend/services/geo is the row's legacy binding): `validate_location_fact` fails closed on unknown role/region_type/precision_class/precision_state and on a `precise`/`coarse_cell` claim the fact's evidence cannot support (precision never exceeds evidence); `build_location_edge_intents` emits one typed, evidence-carrying edge per resolution target, each carrying the provenance keys now registered on `edge_properties.py::OPTIONAL_EDGE_PROPERTIES` (`location_role`/`precision_class`/`precision_state`/`region_type`/`coarse_cell`) + the fact's `EvidenceRef`s on edge and intent. Pure assembly — the governed write path (consent, vertex materialisation, soft-revoke leaves) and the projection read land in G4.4/G4.5. Pinned by `tests/graph/test_geographic360_graph_surface.py` (11) + the exhaustiveness/parity gates; full backend suite 7098 passed, 1 skipped. Registry rows unchanged — `geographic360` still `in_flight`. |
| 2026-09-03 | Phase 4 (G4.3 — spatial substrate) | The client-side spatial substrate the G4.2 vocabulary stores its `coarse_cell` strings against lands (14dc8ea7). `services/backend/services/geo/spatial_cells.py` computes h3 (v4 API) `h3`-scheme string cells in-application — `coordinate_to_cell` at the canonical `coarse_cell` resolution (6), center/resolution, strict hierarchical `parent_cell`/`child_cells`, `k_ring` + exact `ring` neighbourhoods, and `contains_cell` hierarchical containment — every failure typed (`CellValidation`/`CoordinateDomain`/`Resolution` under `SpatialCellError`), never a raw h3 exception; cell centres re-index to their own cell (stable, rebuildable, no PostGIS / no spatial index). `services/backend/services/geo/geodesy.py` gives pure-python WGS84 distance via `geographiclib` (`geodesic_distance_m`, `cell_centroid_distance_m`). `services/backend/services/geo/geocoding.py` adds the vault-backed `GeocodingProvider` protocol beside the network-egress `GeoProvider`: `reverse` (coordinate → coarse region/place label + client-computed `coarse_cell`, never a coordinate echo) and `forward` (query → candidate labels); external geocoder keys resolve ONLY through `CredentialService.get` on a tenant-scoped `geo:{provider}` ref (`build_vault_geocoding_provider`) — never env/hardcode, key passed to the executor per call, repr masked; fail-closed to the honest `NullGeocodingProvider` when the credential is missing/revoked or no executor is wired, typed `provider_error`/`invalid_input` hits on executor failure / out-of-domain input, unknown region types normalized to `None` against `REGION_TYPES`. Dependencies `h3>=4.0` + `geographiclib>=2.0` declared in the `backend` optional group (services/backend/services/geo imports them directly). Pinned by `tests/unit/test_geographic360_{spatial_cells,geodesy,geocoding}.py` (34); geo-surface suite stays green. Registry rows unchanged — `geographic360` still `in_flight`. |
| 2026-09-03 | Phase 4 (G4.4a — geographic360 provider) | `services/backend/services/geographic360/provider.py` ships the `geographic360` projection leaf (`summary`/`state`/`timeline`/`evidence`/`findings`) over canonical location facts (55ccc490). A `Geographic360Reader` protocol + default honest-missing `GeographicLocationReader` back it — no location store exists yet (the governed write path + DSR eraser land in G4.5), so only injected readers exercise full semantics. Precision honesty is structural: `LocationRow` carries a stored `precision_class` but the provider recomputes the rendered class from the surviving labels (`_carried_class_index`), so a coarsened render is never mistaken for exact; `exact→city→metro` downgrade (`RENDER_CAP_CITY`/`RENDER_CAP_METRO`, suppression blanks to country) surfaces through the dims `id="precision"` `degraded`/`suppressed`/`missing` states + finding codes (`geographic360.precision_reduced`/`.suppressed`/`.jurisdiction_conflict`/`.conflicting_country`/`.stale_location`/`.subject_unknown`) — never a silent coarsening, never a differential-privacy claim, never a fabricated section-state literal. Coordinates live ONLY in location facts (`coordinate_present: bool`, no lat/lon/accuracy echoed into sections — the no-raw-geo parity invariant). Unknown/empty/stale (STALE_AFTER_DAYS=180 → typed stale freshness)/suppressed stay distinct typed states; reader explosion degrades (never raises); foreign tenant fails closed. Pinned by `tests/unit/test_geographic360_provider.py` (22); full backend suite 7140 passed, 1 skipped. Registry row unchanged — `geographic360` still `in_flight`. |
| 2026-09-03 | Phase 4 (G4.4b — dedicated `geographic360` surface + adapter) | The WHERE leaf's surface-capability block + projection adapter ship (306a80a9). Because `geo` is already owned by the graph-plane `GeoSurfaceAdapter` (country buckets), the no-shadowing rule forces a NEW dedicated `geographic360` surface (sibling pattern of temporal360 T2.3 / population360 P3.5a): registry block (entity/time/geography/truth categories; window/compare/relative modes; map/table views; all flags on) + regenerated PY/TS/MD twins (`EXPLORATION_SURFACE_IDS` 16 → 17) + `Geographic360SurfaceAdapter` registered on the exploration fabric (deps profile360/temporal360 through the S1 seam) — so `geographic360` is routable on `/v1/explore/query` without shadowing `geo`. Surface pinned in `tests/unit/exploration/test_exploration_adapters.py`, `tests/contracts/test_surface_capability_parity.py`, `test_projection_surface_adapter.py` (no-shadow keeps `geo` on `GeoSurfaceAdapter`); 60 tests green across the pinned four suites. Registry row unchanged — `geographic360` still `in_flight` (surfaceIds flip + boot wiring are G4.5). |
| 2026-09-03 | Phase 4 (G4.5 — canonical store + capsule semantics + DSR + `implemented` flip) | geographic360 is the **third context_360 row to flip `in_flight` → `implemented`** (C4 b143e554; store C1 a2ebd71e, capsule semantics C2 48394a10, DSR eraser C3 134dea0d): **C1** ships the ONE canonical `location_facts` store (`services/backend/services/geo/location_facts.py::LocationFactRepository`, `BaseRepository` — in-memory local / asyncpg prod, the same backend the population plane uses) with an *internal* `record` write (no public route/consent surface), tenant-scoped active-fact reads, and a governed soft-revoke (`lifecycle_state active → revoked`, never a hard delete); the default `GeographicLocationReader` becomes store-backed, so a subject with recorded facts projects them and a subject with none reads the honest `missing`. **C2** formalizes the row's pending `context_capsule_semantics` spine as `services/backend/services/geographic360/capsule_semantics.py`: role resolution (semantics → source → `observed_presence`, never a stronger claim), precision derived from carried labels and capped at `coarse_cell` (a capsule has no coordinate, so `precise` is unreachable), a write-side `capsule_location_fact` builder (provenance `provider=context_capsule`, no coordinate, no invented jurisdiction), and a read-boundary guard the default reader applies to capsule-provenance rows. **C3** appends `location_facts` to `DSR_COMPONENTS` (29 → 30) and adds `_erase_location_plane` to the durable `consent.erasure` handler (governed soft-revoke per active fact, receipt = the store's own revoke count). **C4** empties `pendingAuthority` — `context_capsule_semantics` is formalized into the validator `SPINE_INDEX` (the designed formalization step, not a weakening) — converges `legacyBindings` on the dedicated `geographic360` surface (`migrationMode: converged`), regenerates PY/TS/MD twins, registers geographic360 at app mount in `dependencies/projection_plane.py` (six implemented), and updates the boot-wiring + registry-schema suites to the six-implemented set. Pinned by `tests/unit/test_geographic360_location_store.py` (10), `test_geographic360_capsule_semantics.py` (15), `tests/dsr/test_geographic360_dsr_erasure.py` (3); geographic360 unit suites 79 green, DSR suite 76 green. **No `production_ready` claim.** Blueprint reviewed + source-linked docs (`ARCHITECTURE.md`/`BACKEND-API.md`) reviewed and stamped; IPA/EXPLORATION_FABRIC updated to six slices. |
| 2026-09-04 | Phase 5 (X5.1 — context-triad cross-360 composition) | `shared/projection_engine/composition.py` (S6 cross-360 composition) now reaches the three implemented context leaves (81f1aad6): `_MEMBER_OVERLAY_LENS` gains `geographic360`→`geographic` / `population360`→`population` / `temporal360`→`temporal` (real generated lenses, all standard-based), and the new `compose_context_triad` runs geographic360 (WHERE) × temporal360 (WHEN) × population360 (WHO) over one tenant-scoped subject — the Context Intelligence 360 family rule (spatiotemporal analysis is composition of the three projections, never a fourth `Spatiotemporal360` backend) expressed through the engine-composition pattern the §3 row names. The context overlays intersect on the `entity` subject kind, so an entity subject composes all three; a source/population/cluster subject drops whichever member lens cannot apply as a typed `CAPABILITY_MISSING` conflict while the survivors compose — no member content silently lost, no new public write/consent surface. Exported from the engine barrel (`__init__`). Pinned by five new tests in `tests/unit/test_cross_360_composition.py` (entity full-compose union, per-subject-kind typed drops, absent-provider degrade, determinism); projection-engine + cross-360 suites 59 passed. |
| 2026-09-04 | Phase 5 (X5.2 — cross-360 monetary/FX canonical-seam guard) | Codifies the §4 rule-4 measurement invariant that cross-360 monetary metrics consume canonical `value.ts`/FX provenance only — never a geography- or population-specific FX (bd00ace6): `scripts/validate_cross360_monetary_fx.py` scans the context-360 leaves, `services/backend/services/geo`, the exploration path, and the cross-360 composition seam for monetary/FX handling vocabulary — 35 files, zero offenders today, so the committed shrink-only allowlist (`scripts/allowlists/cross360_monetary_fx.json`) is empty and any new offender fails CI with a pointer to the canonical seam. The token set deliberately excludes the canonical-seam names a legitimate path would import (`services.value` / `economic360_contracts`) — those are the remediation, not the offense. Registered in `scripts/repo_doctor.py` beside the financial-value-semantics gate. `docs/source-of-truth/FINANCIAL_VALUE_SEMANTICS.md` gains rule 8 (one FX seam — no geography- or population-specific FX) and lists the new gate. |
| 2026-09-04 | Phase 5 (X5.3 — cross-cutting docs close-out) | Phase 5 §3 row + §3.1 workstream table + this ledger updated; Phase 4 §3 summary row corrected to SHIPPED (the G4.x workstream rows had flipped but the top row had not); `PROJECTION_ENGINE_ARCHITECTURE.md` S6 reviewed to five entry points (the context-triad + the entity subject-kind intersection + the canonical-FX-seam note). Exit gate `make ci-check` green. The context-360 program is complete: all three context rows `implemented`, cross-dimensional composition served through the engine pattern, monetary metrics canonicalized. **No `production_ready` claim.** PR deferred to the post-merge re-cut per lane convention (branch remains behind `origin/main`). |
...
`outcome360`, `infrastructure360`, and the three context-360 rows flipped so far
— `temporal360` (Phase 2), `population360` (Phase 3) and `geographic360`
(Phase 4, G4.5) — with Phase 5 (X5.1–X5.3) closing the program: cross-
dimensional composition of the three context leaves via the engine pattern
(`compose_context_triad`), and cross-360 monetary metrics on the canonical
`value.ts`/FX seam (no geography- or population-specific FX). None of the trio
was actually served live until the Phase-1 registration seam; temporal360 was
not routable on the exploration surface until its T2.3 surface + adapter
shipped, population360 until its P3.5 surface + adapter shipped, and
geographic360 until its G4.4b surface + adapter shipped (with the G4.5
surfaceIds flip + boot wiring converging the row).
