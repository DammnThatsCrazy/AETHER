...
| **3** — Common spine envelope (C2) | `packages/shared/spine-envelope.ts` + backend mirror; parity test; envelope composes existing canonical primitives; no-producer fields (`identity_watermark`, `rights_decision_ref`) declared `@unpopulated`; no producers claimed (superseded 2026-09-14 for `rights_decision_ref` — see the ledger) | Phase 2 landed | Parity green; no redefinition of `EntityRef`/`EvidenceRef`/`PageRequest`/temporal envelope/`ContextCapsule` | SHIPPED (2026-09-03) |
...
| 2026-09-03 | 3 | Common spine envelope: `packages/shared/spine-envelope.ts` + backend mirror (`services/backend/shared/spine/spine_envelope.py`) + parity test; composes canonical primitives; `identity_watermark` / `rights_decision_ref` declared `@unpopulated`; no producers claimed. Lane commit 4e1926c2 |
...
| 2026-09-05 | re-cut | Phases 1–7 re-cut again onto `origin/main` (`9620539f`) and stacked onto PR #608 (`feat/social360-relationship-fidelity`) as a sibling lane to Social360/Relationship-Fidelity, Risk360/Fraud360, and Universal Financial Normalization. Reconciliation against the newer base: the repo-doctor spine gate (validate_spine_registry dispatch + generated spine twins in the clean-check list) and the projection-plane registry-derived `SPINE_INDEX`/`PENDING_SPINE_INDEX` were merged additively onto main's `readonly_generation_workspace` structure (main's hand-maintained hardcoded index superseded — its resolved ids now resolve against the registry). Registry re-formalization: `graph_history_replay`, `grouping_membership`, and `context_capsule_semantics` moved `pending` → `implemented` (now 25 `implemented`, 7 `in_flight`, 2 `pending`; `SPINE_INDEX` 32 resolved / `PENDING_SPINE_INDEX` 2) because their authorities (`services/backend/services/temporal360` history replay, `services/backend/services/population` membership governor + append-only `population_definition_versions`, `services/backend/services/geographic360` capsule semantics) exist on the new base — a `pending` row pointing at built code is a false claim. Ledger + architecture-SOT + projection-architecture docs updated to the reconciled set; generated twins / doc-manifest / REPO-INDEX regenerated. Python-side verification runs against the combined PR #608 tip |
| 2026-09-14 | rights (Phase 3 envelope producer) | The rights-propagation producer ships, superseding the Phase 3/5 envelope columns above: `services/backend/services/rights_authority/propagation.py` resolves through the authoritative `EffectiveRightsResolver` and stamps the returned durable `rdec_...` decision id onto a `MutationIntent` (`stamp_intent`) or a `SpineEnvelope` (`stamp_envelope`). `rights_decision_ref` **leaves** `SPINE_ENVELOPE_UNPOPULATED_FIELDS` in `services/backend/shared/spine/spine_envelope.py` and its TS twin `packages/shared/spine-envelope.ts` (`identity_watermark` is now the only `@unpopulated` field). `MutationRecord.rights_decision_ref` reaches the append-only `graph_mutation_ledger` via additive alembic migration `20260914_graph_mutation_rights_ref` + `GRAPH_MUTATION_LEDGER_RIGHTS_REF_DDL`. First wired pipeline: `services/backend/services/population/governance.py` (`PopulationMembershipGovernor.add_membership`), the same membership edge the Phase-5 re-cut re-formalized the `grouping_membership` spine around. Gate: `RIGHTS_AUTHORITY_ROLLOUT` off ⇒ the producer returns before touching the resolver and the write stays byte-identical; a denial is never stamped |
