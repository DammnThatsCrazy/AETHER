...
### Agentic governance and release evidence

Kyber exposes tenant-scoped governance endpoints for Agentic Intelligence:

- `GET /v1/admin/kyber/agentic-observability/commercialization` — usage dimensions, entitlement keys, and limits.
- `POST /v1/admin/kyber/agentic-observability/privacy/dsr-preview` — DSR export/tombstone impact preview and optional redacted rows.
- `GET /v1/admin/kyber/agentic-observability/security-privacy` — no-execution, secret handling, privacy, and cross-tenant controls.
- `GET /v1/admin/kyber/agentic-observability/rollout-controls` — feature flags, emergency disable, and rollback notes.
- `GET /v1/admin/kyber/agentic-observability/operator-audit-package` — support/audit bundle.
- `GET /v1/admin/kyber/agentic-observability/release-candidate-evidence` — tenant scenario evidence across pipeline, product surfaces, and governance controls.

All endpoints are observation-only and never execute provider actions.
