...
## Kyber reconciliation diagnostics

`services/agentic_observability/reconciliation.py` provides tenant-scoped
pipeline diagnostics for Kyber. Operators can inspect aggregate pipeline health,
look up a single event's lineage across Bronze, Silver, canonical activity, and
outbox, and run read-only reconciliation to detect missing stages. The service
reports gaps such as `silver_missing`, `canonical_activity_missing`, and
`graph_outbox_missing`; it does not replay records or mutate external providers.
