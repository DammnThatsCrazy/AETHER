...
toc_depth: 3
source_hashes:
  ".github/workflows/": "sha256:379d39fb27810e013f0cdc0973c1787a86b30e543f94b5b70d6cc713f760f0f5"
...
| `amplify-status-production.yml` | push to `main`; `workflow_dispatch` | Waits for the exact main integration authority, binds the public `aether-status` Amplify app to the repository, and deploys the exact main SHA to its `PRODUCTION` `main` branch. If the historical app is still a manual, unconnected deployment, it removes only that app's legacy branch metadata and recreates the branch after binding; repository-backed reruns never delete branches. It verifies the runtime links and the AVAILABLE verified production status hostname. | no |
