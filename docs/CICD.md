...
| `amplify-status-production.yml` | push to `main`; `workflow_dispatch` | Waits for the exact main integration authority, binds the public `aether-status` Amplify app to the repository, and deploys the exact main SHA to its `PRODUCTION` `main` branch. If the historical app is still a manual, unconnected deployment, it moves any live status-domain mapping to a workflow-owned temporary branch, removes only that app's legacy branch metadata, recreates the canonical branch after binding, restores the domain mapping to `main`, and removes the temporary branch; repository-backed retries only repair an interrupted migration and never delete normal branches. It verifies the runtime links, the AVAILABLE status association, and the live CNAME target for the production status hostname. The status/pilot path disables Amplify auto-subdomain creation but explicitly passes the exact empty delegated Amplify role required by the API's dependent authorization check; the staging policy constrains that pass to `amplify.amazonaws.com`. The role has no permissions or Route 53 access, and Squarespace remains authoritative. | no |
