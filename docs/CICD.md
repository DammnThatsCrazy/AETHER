...
toc_depth: 3
source_hashes:
  ".github/workflows/": "sha256:91c32c22d2f51861b38ad3bec92b96c268d56fdb76f31f23973979c6fa21b9af"
...
The full-lane release requires the Kyber Google workforce contract and passes
only the public client identifier into the Kyber bundle; its client secret is
backend-only and is mounted from Secrets Manager at task start. The pilot lane
records Kyber workforce identity as explicitly deferred: Google/GCP credentials
are not read, and the lifecycle does not publish or probe Kyber. Full-lane
build and promotion gates still fail closed when workforce credentials are
required but absent.
...
re-plans. For `deployment_lane=pilot`, policy validation runs both when the
reviewed plan is produced and immediately before apply. It rejects ECS service
or scaling-target replacement, Application Auto Scaling ownership-tag drift,
destructive Aether Auth0 changes, and Auth0 changes outside the Aether pilot
path; its shared database-connection step appends only Aether and leaves the
existing client set outside Terraform's pilot write scope. The guard permits
only non-destructive state-only release of legacy full-set addresses. Staging
wake and sleep use the same guard.
...
overwriting the key.
For a rehearsal, a second pre-wake gate validates both the delivery and
rehearsal credential contracts, assumes `AetherStagingDeploy` from the staging
GitHub environment, checks live ECR image-pull authorization and the registered
lane-specific ECS secret mounts, verifies the exact staging DynamoDB cache
table and every live task role's required cache permissions, and proves the
pilot GitHub-secret persistence path with a disposable secret. The reviewed
wake plan is not dispatched unless these checks succeed. After Terraform apply,
the promotion workflow repeats the cache-role check against the applied
Terraform outputs. The deploy workflow also revalidates the bounded awake lease
immediately before each static S3 write so a lease expiring during readiness
or upload cannot authorize publication.
...
dispatches canonical delivery with `rehearsal_run_id` set to its own run. Delivery
...
For a staging full-rehearsal, release validation precedes the reviewed wake
plan: the lifecycle requires a successful immutable build for the exact current
`main` SHA, verifies the manifest checksum, staging profile/lane, all packaged
SPA/migration/configuration digests, and pilot deferral evidence, then checks
the delivery/rehearsal credentials and live delivery-role/task-definition
contracts. The later canonical deploy repeats artifact verification before
any migration, ECS rollout, or static-origin write; rehearsal consumes that
...
the exact release bytes without publishing them a second time. This keeps
Kyber internal and prevents a public marketing build from being mistaken for
...
reviewed Terraform plan/apply, wake/sleep lifecycle and smoke gate. The staging
lifecycle defaults to `pilot`; the multi-profile `terraform-promote.yml`
workflow keeps a `full` default so production/demo/preview dispatches remain
valid, and a direct staging promotion must explicitly select `pilot`. The
...
The staging state-reconciliation workflow also exposes a separate
`IMPORT-LEGACY-STAGING-AURORA` confirmation for the pre-existing `aether-staging`
cluster. It validates the exact live cluster identity and imports Terraform
state only; it does not change the database. A fresh ordinary reviewed plan is
required afterward, and the plan policy permits only the approved 0–2 ACU,
300-second auto-pause update (or a no-op) with no unrelated cluster changes.
The imported cluster's existing global/local write-forwarding and final-snapshot
settings are explicitly preserved so provider defaults cannot widen that
update. A provider-reported `0` auto-pause value is accepted only in the
before-state; the desired state must still specify 300 seconds.
