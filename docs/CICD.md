...
toc_depth: 3
source_hashes:
  ".github/workflows/": "sha256:85811f03c0c6924ec565d5a73a38c7e11713b56873cd2db87dde28017820440d"
...
| `amplify-status-production.yml` | push to `main`; `workflow_dispatch` | Waits for the exact main integration authority, binds the public `aether-status` Amplify app to the repository, and deploys the exact main SHA to its `PRODUCTION` `main` branch. If the historical app is still a manual, unconnected deployment, it moves any live status-domain mapping to a workflow-owned temporary branch, removes only that app's legacy branch metadata, recreates the canonical branch after binding, restores the domain mapping to `main`, and removes the temporary branch; repository-backed retries only repair an interrupted migration and never delete normal branches. It verifies the runtime links and the AVAILABLE verified production status hostname. Before any domain mutation, it verifies the dedicated empty `AETHER-staging-amplify-domain-role` and passes only that role to Amplify; its IAM grant uses the reviewed `ForAnyValue:StringEquals` `iam:PassedToService` restriction; Squarespace remains authoritative and auto-subdomain creation stays disabled. | no |
