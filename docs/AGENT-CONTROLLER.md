...
status: stable
since_version: 0.1.0
source_files: [services/agents/agent_controller/governance.py, services/agents/agent_controller/nous.py, services/agents/agent_controller/runtime/cycle_runtime.py]
canonical_owner: agent@aether
estimated_read_minutes: 12
toc_depth: 3
source_hashes:
  services/agents/agent_controller/governance.py: sha256:6b1fb5c987f2a7b20d457f015910020e1d2d7f726454b4b3a22185380e65e5b4
  services/agents/agent_controller/nous.py: sha256:7ba21f17eec0d43e6eef3cb0f5cd01d7735ed75540b96bb6423fc5aae274e0a8
  services/agents/agent_controller/runtime/cycle_runtime.py: sha256:b0e78d62cbfdb594fc4b0e98972fa5bde9e74cd50bb7162d109e5b7cb46ed551
---
...
### Nous Controller
Top orchestration controller across all domain controllers. Nous coordinates, synthesizes, supervises, routes, verifies, recovers, and communicates — but **never** directly acts like a worker, mutates the graph, bypasses review, or replaces domain controller responsibilities.

### Intake Controller
First-contact for new objectives. Normalizes input, deduplicates, classifies severity, and routes admitted objectives to Nous.
...
Cycle is aggressive from day one — it continues existing objectives, reopens unresolved ones, creates low-risk maintenance objectives when policy allows, and revisits stale graph areas.

### Cycle Permissions
Cycle **may**: continue objectives, reopen unresolved objectives, create low-risk maintenance objectives, revisit stale areas, sleep when idle, pause when blocked.

Cycle **may not**: create unrestricted objectives, bypass Governance, bypass Nous, bypass verification, bypass staged review, commit directly, spin forever.
...
Three modes:
1. **Pure work mode** — Atoms disabled, no identity rendering
...
Atoms applies by default to controllers and teams. Optional for objectives and workers.
...
3. **Controller Health Console** — controller status, Cycle state, Catalyst fires
