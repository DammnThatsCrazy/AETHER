...
Playbook evaluation selects a recommendation family from the tenant playbook category and delegates generation to the same `RecommendationFamilyRegistry`. The generated recommendation remains explainable, governed, and compatible with investigation, decision, action, outcome, and ledger APIs. See [Playbooks](./PLAYBOOKS.md).
