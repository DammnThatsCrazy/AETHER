// =============================================================================
// Aether SDK Proof Harness — app shell (apps/proof-ios)
// 8 tabbed screens wired to real SDK functions. SDK key from
// PROOF_IOS_SDK_KEY env or a default staging key.
// =============================================================================
...
/** Resolve the SDK key: PROOF_IOS_SDK_KEY env, else a default staging key. */
function resolveSdkKey(): string {
  // In a real Expo build, process.env is injected by the bundler / env plugin.
  // For the proof harness we also accept a default so the app is always usable.
...
function DebugConsoleScreen(): React.ReactElement {
  // We keep a ref so the component re-renders when state changes.
  const [, setTick] = useState(0);
  const tick = useRef(0);

  // Poll the SDK state every 500ms so the console updates as events are sent.
  // In a real app you'd use a subscription; here a simple interval is fine.
  React.useEffect(() => {
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={{ padding: 12, marginTop: 8 }}>
        <TouchableOpacity
          ...
          onPress={() => {
            const s = getState();
            // Force a re-render by bumping the tick.
            tick.current += 1;
            setTick(tick.current);
          }}
        >
...
function ScreenTrackingScreen(): React.ReactElement {
  ...
  const handleTrack = useCallback(async () => {
    setLoading(true);
    const s = await trackScreen('HeartbeatScreen', { source: 'proof-ios' });
    setLast(s);
    setLoading(false);
  }, []);
        ...
        {Object.keys(TABBED_SCREENS).map((name) => (
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              // Track the screen tap as an event too (proof that trackScreen
              // works from the tab bar).
