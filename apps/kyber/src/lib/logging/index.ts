...
function emit(level: LogLevel, message: string, data?: unknown): void {
  ...
  switch (level) {
    case 'debug':
      console.debug(`[KYBER] ${entry.message}`, data ?? '');
      break;
    case 'info':
      console.info(`[KYBER] ${entry.message}`, data ?? '');
      break;
    case 'warn':
      console.warn(`[KYBER] ${entry.message}`, data ?? '');
      break;
    case 'error':
      console.error(`[KYBER] ${entry.message}`, data ?? '');
      break;
  }

  // Hook for external error reporting (Sentry, DataDog, etc.)
  if (level === 'error' && typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('kyber:error', { detail: entry }));
  }
}
