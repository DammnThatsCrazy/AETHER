import { isLocalMocked, getEnvironment, getRuntimeMode } from '@kyber/lib/env';
import * as fixtures from '@kyber/fixtures';
...
export function useLabData() {
  ...
  function exportAsJson(name: string, data: unknown) {
    ...
    a.href = url;
    a.download = `kyber-${name}-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
