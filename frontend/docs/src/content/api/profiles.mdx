...
Read a resolved [profile](/doc/concepts/profiles) — the composed 360 view of
one entity, or any single facet of it — with a read-scoped API key.

## Authentication

`Authorization: Bearer <read_key>` with `read` permission. See
[Authentication](/doc/api/authentication).
...
curl -H "Authorization: Bearer $AETHER_READ_KEY" \
  "https://api.olympuslabsml.com/v1/profile/usr_18f2?include_lake=false&timeline_limit=100"
```
...
| `GET /v1/profile/{user_id}/journeys` / `/unified-journey` | Journey history (see [Journeys](/doc/concepts/journeys)) |
...
- [Profiles concept](/doc/concepts/profiles) — identity resolution and how a
  profile is composed.
- [Lenses](/doc/concepts/lenses) — composable views over the same
  underlying subject.
