...
# Shopify Integration Guide <Available />
...
identity resolution (see [Profiles](/doc/concepts/profiles)) can link a
Shopify order back to the anonymous session and campaign touchpoints that
...
`attribution` [lenses](/doc/concepts/lenses) surface that attribution
weight once the order is in the graph.

## Next steps

- [Stripe integration](/doc/tutorials/stripe-integration) — payment-level
  revenue data alongside order-level Shopify data.
- [Sources](/doc/concepts/sources) — how connectors relate to SDK sources.
