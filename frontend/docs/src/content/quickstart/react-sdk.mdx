...
[Web SDK quickstart](/doc/quickstart/web-sdk) with a provider and a handful
of hooks — `AetherProvider`, `useAether`, `useIdentity`, `useConsentState`,
and `useScreenOrPageTracking`.
...
[Web SDK guide](/doc/quickstart/web-sdk) are all available.
...
- [Node SDK quickstart](/doc/quickstart/node-sdk) — mirror your client-side
  events with server-side observation for API requests, jobs, and webhooks.
- [Journeys](/doc/concepts/journeys) — `useJourneyResumed` and the
