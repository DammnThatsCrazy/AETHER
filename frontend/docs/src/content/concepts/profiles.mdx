...
for callers that only need one slice. See [Profiles API](/doc/api/profiles).
...
- [Journeys](/doc/concepts/journeys) — the step-by-step paths a profile
  takes through your product.
- [Lenses](/doc/concepts/lenses) — composable views over a profile (or any
  graph subject).
- [Profiles API](/doc/api/profiles) — the full endpoint reference.
