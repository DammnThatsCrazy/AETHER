...
[signal](/doc/concepts/signals) is a single event, a journey groups a
sequence of them under one lifecycle so you can ask "how far did users get,
and where did they drop off" instead of reconstructing that from raw events
yourself.
...
that starts on one device can resume on another. When identity resolution
(see [Profiles](/doc/concepts/profiles)) recognizes a returning visitor, the
SDK calls `resumeJourney('identity_resolved', …)` with the handoff latency
and confidence signals attached — so a journey that started on mobile and
finished on desktop shows up as one continuous journey, not two unrelated
fragments.

## Reading journeys back

`GET /v1/profile/{user_id}/journeys` and `/unified-journey` on the
[Profiles API](/doc/api/profiles) return a profile's journey history;
`journey` is also available as an overlay [lens](/doc/concepts/lenses) for
viewing any graph subject through its journey structure.

## Next steps

- [Signals](/doc/concepts/signals) — the events a journey groups together.
- [Lenses](/doc/concepts/lenses) — the `journey` overlay lens and how lenses
  compose.
