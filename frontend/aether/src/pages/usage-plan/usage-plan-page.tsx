...
export function UsagePlanPage() {
  ...
  const knownDimensions = [
    'sdk_event_ingested', 'webhook_ingested', 'event_ingested', 'connector_sync',
    'graph_operation', 'profile_query',
    'recommendation_generated', 'recommendation_previewed', 'decision_recorded', 'confidence_updated',
    'action_logged', 'action_dispatched', 'investigation_opened',
    'outcome_observed', 'playbook_run', 'audit_export_generated', 'value_created',
  ];
  const dimensions = knownDimensions.filter(dim => usage[dim] != null || included[dim] != null);
  return <main className="p-6 space-y-4"><div><h1 className="text-xl font-mono font-bold">Usage & Plan</h1><p className="text-sm text-text-secondary">Customer-safe view of your package, enabled modules, included usage, current usage, and value-created metrics.</p></div>
    <div className="grid gap-4 lg:grid-cols-3">
      <Card><CardHeader><CardTitle>Current package</CardTitle></CardHeader><CardContent className="space-y-2"><div className="text-2xl font-semibold">{plan.package_id ?? plan.plan_tier ?? 'No package assigned'}</div>{plan.billing_period ? <Badge>{plan.billing_period}</Badge> : <span className="text-sm text-text-muted">Billing period unavailable</span>}<div className="text-sm text-text-secondary">Status: {plan.contract_status ?? 'unavailable'}</div></CardContent></Card>
    ...
    <Card><CardHeader><CardTitle>Included vs current usage</CardTitle></CardHeader><CardContent className="space-y-4">{dimensions.length === 0 ? <EmptyState title="Usage has not been measured" description="Usage dimensions appear after the backend records observations." /> : dimensions.map(dim => <div key={dim}><div className="mb-1 flex justify-between text-sm"><span>{dim.replace(/_/g, ' ')}</span><span>{usage[dim] ?? '—'} / {included[dim] ?? '—'}</span></div>{usage[dim] != null && included[dim] != null ? <UsageBar label={dim.replace(/_/g, ' ')} used={Number(usage[dim])} total={Number(included[dim])} showUpgradeCta={false} /> : <p className="text-xs text-text-muted">Measurement or entitlement unavailable</p>}</div>)}</CardContent></Card>
