...
import { ProjectionSurfacePanel, ProjectionSurfaceSummary } from '@aether-app/features/projection-360';
import type { ProjectionSurfaceSummary as ProjectionSurfaceSummaryModel } from '@aether-app/features/projection-360/projection-360-types';
...
describe('ProjectionSurfacePanel (gated over the typed exploration transport)', () => {
  it('re-targets the context to the projection surface and renders the server summary', async () => {
    const explorationClient = client(availableSummary(), 'outcome360', 'tenant-360-aether-panel');

    render(
      <MemoryRouter>
        <TimeProvider>
          <ExplorationProvider
            tenantId="tenant-360-aether-panel"
            surface="campaign360"
  ...
  it('fails closed on a server-declared unavailable envelope', async () => {
    ...
    render(
      <MemoryRouter>
        <TimeProvider>
          <ExplorationProvider
            tenantId="tenant-360-aether-unavail"
            surface="campaign360"
            client={explorationClient}
          >
