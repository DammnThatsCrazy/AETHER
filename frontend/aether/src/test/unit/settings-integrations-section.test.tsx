...
const item = (
  partial: Partial<TenantIntegrationItem>,
...
describe("Settings → Integrations section", () => {
  ...
  it("renders an honest unavailable state when the read model is unreachable", () => {
    ...
    expect(
      screen.getByText(/SDK ingestion is always available/),
    ).toBeInTheDocument();
  });
  ...
  it("surfaces manifest catalog baseline captions without claiming tenant readiness", () => {
    ...
    renderSection();
    expect(
      screen.getByText(/Catalog: awaiting provider activation/),
    ).toBeInTheDocument();
  ...
  it("routes advertising rows into the ad connect flow while other rows stay on the connector manager", () => {
    ...
    const adRow = screen
      .getByText("Google Ads")
      .closest('[data-provider-family="google_ads"]');
    const nonAdRow = screen
      .getByText("Shopify")
      .closest('[data-provider-family="shopify"]');
    expect(adRow).not.toBeNull();
    expect(nonAdRow).not.toBeNull();

    const adLink = within(adRow as HTMLElement).getByRole("link", {
      name: "Connect",
    });
    ...
    const nonAdLink = within(nonAdRow as HTMLElement).getByRole("link", {
      name: "Manage",
    });
    expect(nonAdLink).toHaveAttribute(
      "href",
  ...
  it("forwards a validated incoming return target through the ad connect deep link (reconciled bridge)", () => {
    ...
    renderSection(
      "/settings/integrations?return=/campaign-intelligence/sources",
    );
    const adRow = screen
      .getByText("Google Ads")
      .closest('[data-provider-family="google_ads"]');
  ...
  it("never forwards an off-origin return target — falls back to the Settings default", () => {
    ...
    renderSection(
      "/settings/integrations?return=https%3A%2F%2Fevil.example%2Fphish",
    );
    const adRow = screen
      .getByText("Google Ads")
      .closest('[data-provider-family="google_ads"]');
...
/**
 * WS-5 marketing→app connect handoff consumer: the section honors a
 * `/settings/integrations?family=…&experience=…&intent=connect|manage` deep
 * link by transiently focusing the named provider's row (data-handoff-focus +
 * ring + one scrollIntoView) and, when the visitor came to connect a provider
 * that is not yet engaged, showing a small inline callout above that group —
 * never auto-navigating and never fabricating connection state.
 */
describe("Settings → Integrations marketing connect handoff", () => {
  ...
  const renderHandoff = (search: string) => {
    useTenantIntegrations.mockReturnValue({
      data: {
        tenant_id: "t1",
        count: 1,
    ...
    return renderSection(`/settings/integrations${search}`);
  };

  it("focuses the named provider row, calls scrollIntoView once, and shows the connect callout", () => {
    renderHandoff(
    ...
    expect(row).toHaveAttribute("data-handoff-focus", "true");

    expect(
      screen.getByText("You came to connect Google Ads — continue below"),
    ).toBeInTheDocument();
  ...
  it("focuses the row for a manage deep link but does not show the connect callout", () => {
    renderHandoff(
      "?family=google_ads&experience=advertising_campaigns&intent=manage",
    );
    ...
    expect(row).toHaveAttribute("data-handoff-focus", "true");
    expect(
      screen.queryByText(/You came to connect Google Ads/),
    ).not.toBeInTheDocument();
  });

  it("treats a junk intent as absent (focus still applies, callout does not)", () => {
    renderHandoff(
  ...
  it("focuses nothing and renders no callout for an unknown family", () => {
    renderHandoff("?family=not_real&intent=connect");

    expect(document.querySelector('[data-handoff-focus="true"]')).toBeNull();
    expect(screen.queryByText(/You came to connect/)).not.toBeInTheDocument();
    expect(Element.prototype.scrollIntoView).not.toHaveBeenCalled();
  });

  it("never shows a connect callout for a provider that is already engaged", () => {
    useTenantIntegrations.mockReturnValue({
      data: {
        tenant_id: "t1",
        count: 1,
