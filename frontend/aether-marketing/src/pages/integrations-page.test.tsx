...
describe('IntegrationsPage', () => {
  it('renders the /integrations hero copy from the SECTIONS entry', () => {
    renderPage();
    expect(
      screen.getByRole('heading', { level: 1, name: /connectors with truthful availability states/i }),
    ).toBeTruthy();
  });
  ...
  it('shows a result count and the full registry by default', () => {
    ...
    for (const connector of CONNECTORS) {
      expect(screen.getByRole('heading', { name: connector.name })).toBeTruthy();
    }
  });
  ...
  it('shows the real status vocabulary for the current registry states', () => {
    ...
    expect(screen.getAllByText(/credentials required/i).length).toBeGreaterThan(0);
  });
  ...
  it('shows a calm empty state with a clear-filters control for a nonsense query', () => {
    ...
    expect(resultCount()).toContain(`${CONNECTORS.length} of ${CONNECTORS.length} connectors`);
  });

  it('points the entry CTA at the documentation and the secondary at the public sign-up threshold', () => {
    renderPage();
    const docs = screen.getByRole('link', { name: 'Read the integration documentation' });
