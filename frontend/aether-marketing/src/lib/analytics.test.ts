...
import { resolveAnalytics } from '@aether-marketing/lib/analytics';
...
describe('resolveAnalytics', () => {
  ...
  it('enables plausible only when the property id is non-empty', () => {
    expect(resolveAnalytics({ provider: 'plausible', propertyId: 'aether.olympuslabsml.com' })).toEqual({
      enabled: true,
      provider: 'plausible',
      propertyId: 'aether.olympuslabsml.com',
    });
  });
