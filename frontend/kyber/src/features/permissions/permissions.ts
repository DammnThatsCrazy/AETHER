import type { KyberRole, ActionClass, AutomationPosture } from '@kyber/types';
...
const ROLE_MAX_ACTION_CLASS: Record<KyberRole, ActionClass> = {
  kyber_executive_operator: 4,
  kyber_engineering_command: 5,
  kyber_specialist_operator: 2,
  kyber_observer: 0,
};

const ROLE_PERMISSIONS: Record<KyberRole, {
  readonly canApprove: boolean;
  readonly canIntervene: boolean;
...
export function getMaxActionClass(role: KyberRole): ActionClass {
  return ROLE_MAX_ACTION_CLASS[role];
}

export function canPerformAction(role: KyberRole, actionClass: ActionClass, posture?: AutomationPosture): PermissionCheck {
  const maxClass = getMaxActionClass(role);
  const currentPosture = posture ?? (env.VITE_AUTOMATION_POSTURE as AutomationPosture);
  ...
  if (env.VITE_KYBER_ENV === 'production' && actionClass > 0) {
...
export function usePermissions() {
  const { user } = useAuth();
  const role = user?.role ?? 'kyber_observer';
  ...
  return {
    ...
    checkAction: (actionClass: ActionClass) => canPerformAction(role, actionClass),
  };
}
