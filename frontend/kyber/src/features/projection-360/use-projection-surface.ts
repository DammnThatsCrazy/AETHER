/**
 * Hook over the typed exploration transport for a projection surface (Kyber).
 *
 * Mirrors the exploration-fabric precedent: the shared provider supplies the
 * tenant-scoped base context + authenticated exploration client; this hook
 * re-targets the context to the requested projection surface (outcome360 /
 * economic360 / infrastructure360) and optionally focuses the projection
 * subject, then reads the server-computed projection summary through
 * queryLatest. The client never computes anything — it only names the typed
