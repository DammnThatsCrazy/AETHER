...
export function useRecommendationObservability(window: KyberWindow = '30d') {
  ...
  const integrationHealth = useQuery({
    key: `kyber:integration-health:${window}`,
    fetcher: () => api.admin.kyber.integrationHealth(window),
    staleTime: STALE,
    enabled,
  });
  ...
  const queries = [strategicOverview, tenantValueHealth, familyPerformance, playbookPerformance, modelConfidenceDrift, verticalSolutionSignals, integrationHealth, revenueOpportunities];
  return {
    enabled,
    strategicOverview,
    tenantValueHealth,
