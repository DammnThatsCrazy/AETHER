...
export const noesis = {
  async query(input: NoesisQueryInput): Promise<NoesisResponsePayload> {
    const response = await restClient.post('/v1/noesis/query', noesisResponseSchema, requestBody(input, 'kyber'));
  ...
  async streamQuery(
    ...
    const response = await fetch('/v1/noesis/query/stream', {
      method: 'POST',
      headers,
      body: JSON.stringify(requestBody(input, 'kyber')),
    });
    ...
    while (true) {
      ...
      for (const event of parseSseChunk(ready)) {
        onEvent(event);
        if (event.type === 'error') throw new Error(event.data.message || event.data.code || 'Noesis stream failed');
...
export async function exportNoesisConversations(tenantId?: string): Promise<unknown> {
  const query = new URLSearchParams({ surface: 'kyber' });
  if (tenantId) query.set('tenant_id', tenantId);
  const response = await restClient.get(`/v1/noesis/conversations/export?${query.toString()}`, noesisResponseSchema);
  return response.data;
}

export async function deleteNoesisConversation(conversationId: string, tenantId?: string): Promise<void> {
  const query = new URLSearchParams({ surface: 'kyber' });
  if (tenantId) query.set('tenant_id', tenantId);
  await restClient.delete(`/v1/noesis/conversations/${conversationId}?${query.toString()}`, noesisResponseSchema);
}
