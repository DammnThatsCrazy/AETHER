...
import { GraphInspector } from '@kyber/components/graph/graph-inspector';
import { GraphToolbar } from '@kyber/components/graph/graph-toolbar';
...
export function NoesisGraphExplorerPage() {
  ...
  useEffect(() => {
    if (!deepLinkedNodeId || isLoading) return;
    const node = nodes.find(n => n.id === deepLinkedNodeId);
    if (!node) return;
    setViewMode('graph');
