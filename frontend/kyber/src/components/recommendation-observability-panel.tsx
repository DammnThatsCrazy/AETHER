...
export function RecommendationObservabilityPanel() {
  ...
  const integrationHealth = items(data.integrationHealth.data).slice(0, 5);
    ...
    <Card>
      ...
      <CardContent>
        ...
        <Section title="Integration health">
          {integrationHealth.length === 0 ? <Empty label="No integration dispatch health yet." /> : integrationHealth.map((integration) => (
            <Row key={text(integration.tenant_id)} title={text(integration.tenant_id)} badge={`${numberText(integration.dispatch_volume)} dispatches`}>
              <span>Success {pct(integration.dispatch_success_rate)}</span>
              <span>Failures {pct(integration.dispatch_failure_rate)}</span>
              <span>Manual/integrated {numberText(integration.manual_to_integrated_ratio)}</span>
              <span>Metered ${numberText(integration.estimated_billable_value)}</span>
              <span>{text(integration.recommended_olympus_action)}</span>
            </Row>
          ))}
        </Section>
