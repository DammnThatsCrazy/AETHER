  ...
  return (
    <Card className={cn('flex flex-col h-full', className)}>
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        <Button variant="ghost" size="sm" onClick={onClose} aria-label="Close path inspector">
          ✕
        </Button>
      </CardHeader>
          ...
          <TabsContent value="overview" className="flex-1 overflow-auto px-4 pb-4 space-y-4">
            <div className="flex flex-wrap gap-2 pt-2">
              ...
              {path.layer_sequence.map((layer, i) => (
                <Badge key={i} variant="default" aria-label={`Layer: ${layer}`}>
                  {layer}
                </Badge>
              ))}
            </div>
            ...
            <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
              ...
              <dd className="font-mono">{formatDateTime(path.computed_at, timeCtx)}</dd>
            </dl>
          </TabsContent>
              ...
              <ol className="space-y-2 pt-2">
                {path.nodes.map((node, i) => (
                  <li key={node.id} className="flex items-start gap-2">
                    <span className="shrink-0 mt-0.5 w-5 h-5 rounded-full bg-surface-raised flex items-center justify-center text-xs font-mono">
                      {i}
                    </span>
                    <div className="min-w-0">
                      <div className="flex items-center gap-1 flex-wrap">
                        ...
                        <span className="text-xs text-text-primary truncate" title={node.id}>
                          {node.label ?? node.id}
                        </span>
                      </div>
                      {i < path.edges.length && (
                  ...
                  <>
                      ...
                      <div>
                        ...
                        <ul className="text-xs text-text-secondary space-y-1">
                          {explanation.supporting_evidence.map((e, i) => (
                            <li key={i} className="truncate">{JSON.stringify(e)}</li>
                          ))}
                        </ul>
                      </div>
                    )}
                    {explanation.contradictory_evidence.length > 0 && (
                      <div>
                        ...
                        <ul className="text-xs text-text-secondary space-y-1">
                          {explanation.contradictory_evidence.map((e, i) => (
                            <li key={i} className="truncate">{JSON.stringify(e)}</li>
                          ))}
                        </ul>
                      </div>
      ...
      {onSaveToInvestigation && (
        <div className="px-4 pb-4 pt-2 border-t border-border">
          <Button
            className="w-full"
            size="sm"
            onClick={() => onSaveToInvestigation(path.path_id)}
            aria-label={`Save path ${shortId} to investigation`}
          >
