...
from services.derivatives.credentials import (  # noqa: E402
    CredentialNotUsable,
...
from services.stablecoins.registry import (  # noqa: E402
...
def test_wrong_environment_registry_separation():
    ...
    # The sepolia twin is testnet + unverified; a mainnet registry never
    # resolves it — staging and production are distinguishable registries.
    staging_sepolia = (
        PLATFORM_STABLECOIN_REGISTRY_STAGING.deployments[
    ...
    assert "0x1c7d4b196cb0c7b01d743fbc6116a902379c7238" not in (
        d.contract_or_mint for d in PLATFORM_STABLECOIN_REGISTRY.deployments.values()
    )
...
async def test_repair_resets_dead_lettered_receipt_into_recoverable_pipeline():
    ...
    # Operator replay: reset_repair moves it back to a recoverable state.
    repaired = await repo.reset_repair("t1", rid, reason="key rotation applied")
    assert repaired["current_stage"] == ReceiptState.REPAIR_PENDING
    assert repaired["repair_attempts"] == 0
    assert repaired["last_error_classification"] is None
    ...
    # worker is the documented escape hatch; reset_repair is what makes replay
    # safe. The advance-from-terminal gap is flagged for the integration pass
