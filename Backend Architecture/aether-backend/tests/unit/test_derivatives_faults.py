...
from services.derivatives.credentials import (
    CredentialNotUsable,
...
def test_reconnect_exhaustion_marks_disconnected_out():
    ...
    assert stream.last_cursor == 2  # restart resumes here


def test_cooperative_shutdown_stops_cleanly_with_partial_cursor():
    stop = {"flag": False}
    ...
    assert result.completed is False
    assert stream.last_cursor is not None and stream.last_cursor > 1


def test_asyncio_cancellation_records_partial_result_and_reraises():
    ...
    assert stream.last_result is not None
    assert stream.last_result.cancelled is True
    assert stream.last_cursor is not None and stream.last_cursor >= 2
...
async def test_build_read_only_adapter_binds_credential_and_auth_headers():
    ...
    # The build_request seam carries the resolved Authorization header.
...
def test_product_meter_usage_delegates_to_hook():
    ...
    out = product_service.meter_usage("t1", "backfill_records", Decimal("4"))
    assert out["meter"] == "backfill_records"
...
def test_centralized_futures_is_declared_scaffolded_not_supported():
    ...
    assert "centralized_futures" in SCAFFOLDED_VENUES
    adapters = build_scaffolded_adapters()
    profile = adapters["centralized_futures"].capabilities
    assert profile.scaffolded is True
    assert profile.supported_concepts == ()
    assert profile.to_dict()["scaffolded"] is True
    # It is never reported as a supported/available venue.
    assert "centralized_futures" not in available_venues(adapters)
    report = cross_venue_parity_report(adapters)
    assert "centralized_futures" in report["scaffolded_venues"]
    assert "centralized_futures" not in report["supported_venues"]
