...
def test_kyber_release_readiness_is_blocked_until_full_productization() -> None:
    ...
    assert capabilities["release_level_end_to_end_scenario"]["status"] == "partial"
...
def test_agentic_governance_commercialization_privacy_and_rollout_surfaces() -> None:
    client = _client()
    created = client.post(
        "/v1/observability/agent/events",
        json=_agent_event(
            ...
            actor={"actor_type": "agent", "actor_id": "agent-1"},
            correlation={"campaign_id": "camp-agentic-1", "trace_id": "trace-agentic-1"},
            authorization={"grantor_id": "human-1", "grantee_id": "agent-1", "external_account_id": "acct-1"},
            verification={"verification_status": "provider_confirmed", "external_object_id": "post-1"},
        ),
    )
    assert created.status_code == 201, created.text

    commercialization = client.get("/v1/admin/kyber/agentic-observability/commercialization")
    assert commercialization.status_code == 200, commercialization.text
    commercial_data = commercialization.json()
    assert commercial_data["metered_dimensions"]["agentic_events_ingested"] == 1
    assert commercial_data["metered_dimensions"]["agentic_graph_mutations_queued"] == 2
    assert "agentic_observability" in commercial_data["entitlement_keys"]
    assert commercial_data["billable_status"] == "metered_not_invoiced"

    dsr = client.post(
        "/v1/admin/kyber/agentic-observability/privacy/dsr-preview",
        json={"subject_id": "agent-1", "include_rows": True},
    )
    assert dsr.status_code == 200, dsr.text
    dsr_data = dsr.json()
    assert dsr_data["records_total"] >= 1
    assert dsr_data["hard_delete_supported"] is False
    assert dsr_data["observation_only"] is True
    assert "redacted_rows" in dsr_data

    security = client.get("/v1/admin/kyber/agentic-observability/security-privacy")
    assert security.status_code == 200, security.text
    security_data = security.json()
    assert security_data["no_execution_invariant"]["status"] == "enforced"
    assert security_data["cross_tenant_isolation"]["status"] == "enforced"

    rollout = client.get("/v1/admin/kyber/agentic-observability/rollout-controls")
    assert rollout.status_code == 200, rollout.text
    rollout_data = rollout.json()
    assert "AGENTIC_OBSERVABILITY_ENABLED" in rollout_data["feature_flags"]
    assert "worker_pause" in rollout_data["emergency_disable"]

    package = client.get("/v1/admin/kyber/agentic-observability/operator-audit-package")
    assert package.status_code == 200, package.text
    package_data = package.json()
    assert package_data["package_type"] == "agentic_observability_operator_audit"
    assert package_data["usage_metering"]["metered_dimensions"]["agentic_events_ingested"] == 1

    evidence = client.get(
        "/v1/admin/kyber/agentic-observability/release-candidate-evidence?agent_id=agent-1&campaign_id=camp-agentic-1"
    )
    assert evidence.status_code == 200, evidence.text
    evidence_data = evidence.json()
    assert evidence_data["release_candidate"] is True
    assert evidence_data["failed_checks"] == []
    assert "provider_confirmed_fact" in evidence_data["evidence_precedence"]


def test_release_readiness_reports_governance_as_partial_not_missing() -> None:
    data = _client().get("/v1/admin/kyber/agentic-observability/release-readiness").json()
    capabilities = {item["capability"]: item for item in data["capabilities"]}
    assert capabilities["kyber_frontend_onboarding_alerts"]["status"] == "partial"
    assert capabilities["security_privacy_billing_rollout"]["status"] == "partial"
    assert capabilities["release_level_end_to_end_scenario"]["status"] == "partial"
    assert data["ga_ready"] is False
