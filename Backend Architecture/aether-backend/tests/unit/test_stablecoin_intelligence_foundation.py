...
def test_deployments_are_chain_specific():
    eth = PLATFORM_STABLECOIN_REGISTRY.resolve(
        chain_id="1",
        network="ethereum-mainnet",
        contract_or_mint="0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48",
    )
    base = PLATFORM_STABLECOIN_REGISTRY.resolve(
        chain_id="8453",
...
def test_observation_requires_tenant_and_execution_identity():
    common = dict(
        ...
        finality_status=FinalityState.FINALIZED,
    ...
    with pytest.raises(ValueError):
        StablecoinObservation(**common, tenant_id="", source_execution_id="exec")
    with pytest.raises(ValueError):
        StablecoinObservation(**common, tenant_id="tenant", source_execution_id="")


def test_gold_identity_separates_tenant_date_asset_deployment_source():
    base = dict(
        tenant_id="t1",
        metric_name="finalized_payment_volume",
        metric_version="v1",
        entity_id="merchant1",
...
async def test_bronze_repeated_executions_are_distinct_and_quarantine_blocks_silver():
    ...
    a, new_a = await bronze.ingest(
        "dune", "tenant:t:stablecoin", "row-1", {"execution_id": "exec-a"},
        tenant_id="t", schema_version="stablecoin.intelligence.v1",
        source_manifest_id="m", license_status="enterprise_contract",
        terms_status="approved",
    )
    b, new_b = await bronze.ingest(
        "dune", "tenant:t:stablecoin", "row-1", {"execution_id": "exec-b"},
        tenant_id="t", schema_version="stablecoin.intelligence.v1:exec-b",
        source_manifest_id="m", license_status="enterprise_contract",
        terms_status="approved",
    )
    ...
    with pytest.raises(ValueError):
        await silver.upsert_record(
            "e", "wallet", "dune", "tenant:t:stablecoin", {},
            bronze_id=quarantined["id"], tenant_id="t", bronze_record=quarantined,
        )
