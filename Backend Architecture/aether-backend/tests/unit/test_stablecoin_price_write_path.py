...
Proves:
  * append-only reorg rollback: ``rollback_execution`` DEMOTES rows (keeps
    evidence, marks ``demoted``) instead of physically deleting them, and appends
    a ``stablecoin_remediation_audit`` entry;
...
# ═══════════════════════════════════════════════════════════════════════════
# Append-only reorg rollback
# ═══════════════════════════════════════════════════════════════════════════

def _observation_row(obs_id: str, **extra) -> dict:
    return {"id": obs_id, "observation_id": obs_id, "tenant_id": "t1",
            "source_execution_id": "exec1", **extra}


def _silver_row(sil_id: str) -> dict:
    return {"id": sil_id, "tenant_id": "t1",
            "source_tag": "tenant:t1:stablecoin:chainlink",
            "source_execution_id": "exec1", "source": "chainlink"}


def _bronze_row(br_id: str) -> dict:
    return {"id": br_id, "tenant_id": "t1",
            "source_tag": "tenant:t1:stablecoin:chainlink",
            "provider_record_id": "exec1:0"}


async def test_rollback_demotes_not_deletes():
    from repositories.stablecoin_repos import (
    ...
    from services.stablecoins.ingestion import StablecoinIngestionPipeline

    obs = StablecoinObservationRepository()
    ...
    await obs.insert("obs1", _observation_row("obs1"))
    await silver.insert("sil1", _silver_row("sil1"))
    await bronze.insert("br1", _bronze_row("br1"))
    checkpoint_id = "stablecoin_checkpoint:t1:chainlink:exec1"
    await checkpoints.insert(checkpoint_id, {"checkpoint_id": checkpoint_id, "status": "ok"})

    runner = StablecoinProviderIngestionRunner(
        observations=obs, silver=silver, bronze=bronze,
        checkpoints=checkpoints, remediation=remediation,
        health=StablecoinProviderHealthRepository(),
        pipeline=StablecoinIngestionPipeline(),
    )
    result = await runner.rollback_execution(
        tenant_id="t1", provider="chainlink", source_execution_id="exec1",
    )

    # Append-only: rows survive, marked demoted.
    ...
    ob = await obs.find_by_id("obs1")
    assert ob is not None
    assert ob["demoted"] is True
    assert ob["demotion_reason"] == "reorg_rollback"

    sl = await silver.find_by_id("sil1")
    assert sl is not None and sl["demoted"] is True

    br = await bronze.find_by_id("br1")
    assert br is not None and br["demoted"] is True

    # Checkpoint marked rolled_back.
    cp = await checkpoints.find_by_id(checkpoint_id)
    assert cp["status"] == "rolled_back"

    # Remediation audit trail appended.
    audit_rows = await remediation.find_many(filters={"tenant_id": "t1"}, limit=100)
    assert len(audit_rows) == 1
    assert audit_rows[0]["action"] == "rollback_demotion"
    assert audit_rows[0]["evidence_preserved"] is True
