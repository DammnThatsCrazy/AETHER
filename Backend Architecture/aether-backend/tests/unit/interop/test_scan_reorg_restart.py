...
DebridgeAdapter and MockEvmRpcServer (NO live network):
  ...
  * malformed logs fail the cycle loudly (main has no per-log decode
    tolerance) and the checkpoint never advances past the undecodable data.
  * dead-letter counter increments when a poison observation is quarantined.
  * build_interop_scan_coro is importable from services.interop.scan_worker and
...
# ── decode-failure fail-closed (PORT-ADAPT: main has no per-log decode
# tolerance — the branch's `_decode_safely`/`_scan_decode_failures` live in the
# superseded OperationalFieldsMixin base adapter that main dropped; main's
# adapter only catches RateLimited, so an undecodable log aborts the scan
# loudly and the checkpoint never advances past it) ────────────────────────

async def test_decode_failure_aborts_loudly_and_never_advances_checkpoint():
    server = MockEvmRpcServer()
    server.set_head(ETH, 100)
    server.set_head(ARB, 100)
    # Malformed log: data too short to decode -> ValueError out of decode_log.
    ...
    assert result["status"] == "error"  # loud, never silent-empty
    assert result["reason"]
    # Fail-closed: no checkpoint advance past the undecodable window and no
    # message row for ANY log in it (nothing fabricated).
    assert await _checkpoints_for("debridge") == []
    assert await _count_messages() == 0


# ── dead-letter counter ──────────────────────────────────────────────────

class _PoisonAdapter(InteropProviderAdapter):
    provider_id = "poison"
    ...
    async def scan(self, checkpoint=None):
        ...
        new_checkpoint = dict(new_checkpoint or {})
    ...
    def operational_state(self, checkpoint=None):
        return {"provider_id": self.provider_id, "configured": False}
...
async def test_graph_projector_wires_topology_mutations():
    # Re-homed: InteropGraphProjector now lives in scan_worker and drives main's
    # services/interop/graph_mutations.build_topology_mutations through
    # foundation.persist_mutations (the canonical graph outbox path).
    from services.interop.providers.debridge import DebridgeAdapter
    from services.interop.scan_worker import InteropGraphProjector
...
async def test_security_snapshot_caller_snapshots_policy():
    # PORT-ADAPT: the branch's free scan_security_policy_snapshots lived in
    # security.py; main carries only the SecurityPolicyService class, so the
    # snapshot-time caller was re-homed into scan_worker (this module).
    from repositories.interop_repos import SecurityPolicySnapshotRepo
    from services.interop.scan_worker import scan_security_policy_snapshots
...
async def test_build_interop_scan_coro_importable_and_is_coroutine():
    ...
    # Re-homed: the poll-loop builder lives in scan_worker (main's lifecycle
    # module carries no worker-spec entry points).
