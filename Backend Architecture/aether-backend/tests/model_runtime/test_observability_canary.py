...
def test_promote_true_when_all_thresholds_met():
    policy = _policy(
        promote_after_samples=10, min_samples=10, max_error_rate=0.05, max_latency_ms=50.0
    )
...
def test_promote_boundary_error_rate_equal_to_max():
    policy = _policy(
        promote_after_samples=10, min_samples=10, max_error_rate=0.1, max_latency_ms=50.0
    )
    tracker = CanaryTracker(policy)
    for _ in range(9):
        tracker.record(tracker.candidate, latency_ms=30.0, ok=True, verified=True)
...
def test_promote_boundary_latency_equal_to_max():
    policy = _policy(
        promote_after_samples=10, min_samples=10, max_error_rate=0.05, max_latency_ms=30.0
    )
    tracker = CanaryTracker(policy)
    for _ in range(10):
        tracker.record(tracker.candidate, latency_ms=30.0, ok=True, verified=True)
...
def test_promote_ignores_latency_when_no_limit():
    policy = _policy(
        promote_after_samples=5, min_samples=5, max_error_rate=0.05, max_latency_ms=None
    )
    tracker = CanaryTracker(policy)
    for _ in range(5):
