...
class _ExplodingSynthesizer:
    """A synthesizer that fails with an unexpected (non-gate) exception."""

    async def synthesize(self, prompt: str, *, plan_kind: str) -> str:
        raise RuntimeError("provider timeout")


async def test_unexpected_synthesizer_error_is_normalized():
    # F12: an unexpected engine/synthesizer failure must NOT leak a raw
    # exception through the public facade; it is normalized into the same
    # content-free SynthesisServiceError as every other engine failure.
    service = SynthesisService()

    await _raises_async(
        SynthesisServiceError,
        lambda: service.synthesize(
            _request(evidence=_evidence_set()), _ExplodingSynthesizer()
        ),
    )


async def test_unexpected_synthesizer_error_preserves_cause_not_message():
    service = SynthesisService()
    ...
    try:
        ...
        assert "provider timeout" not in str(exc)
