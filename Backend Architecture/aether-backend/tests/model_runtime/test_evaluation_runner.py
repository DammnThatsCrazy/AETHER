...
# ---------------------------------------------------------------------------
# F13 — the default runner supplies REAL evidence (grounding gate must pass)
# ---------------------------------------------------------------------------


class _CapturingEngine:
    """Fake engine that records the requests it receives."""
...
async def test_default_runner_supplies_real_evidence():
    # The default evidence builder must seed grounded evidence so the real
    # grounded-synthesis engine (whose grounding gate rejects evidence=None)
    # can execute. Fake engines ignore it, but the request must carry it.
    engine = _CapturingEngine()
    runner = EvaluationRunner(engine=engine)
    case = _case()

    report = await runner.run_case(case, _synth())

    assert report.case_id == "case-001"
    assert len(engine.requests) == 1
    request = engine.requests[0]
    assert request.evidence is not None
    assert len(request.evidence.items) == 1
    assert request.evidence.tenant_id == case.tenant_id
    assert request.evidence.items[0].content == case.expected_ground_truth


async def test_default_runner_runs_real_grounded_engine():
    # End-to-end F13: the default runner with the real grounded-synthesis
    # engine and a faithful synthesizer executes the grounded path (evidence is
    # supplied, so the grounding gate passes) and produces a passing report.
