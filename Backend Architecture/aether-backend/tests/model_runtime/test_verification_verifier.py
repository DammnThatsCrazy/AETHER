...
Coverage: the D7 fail-closed gate. ``run``/``enforce`` semantics, cite-aware
grounding (each claim is checked only against the references it cites), the
zero-claims edges (non-empty output with zero claims is REJECTED; empty output
keeps the leak-only gate because ``all([])`` is ``True``), and leak detection
over content AND citation excerpts.
"""
...
def test_default_engine_full_pipeline_is_faithful():
    ...
    # detector over content/citations. The claim cites [ref:ref-1], so it is
    # checked ONLY against ref-1's excerpt (cite-aware grounding).
    content = "Gold reserves increased [ref:ref-1]."
    ...
    assert vresult.claims == (_claim(text="Gold reserves increased [ref:ref-1]."),)
    assert vresult.checks[0].supported is True
    assert vresult.checks[0].reference_id == "ref-1"
    assert vresult.faithful is True
    assert vresult.leak_detected is False


def test_integration_unsupported_claim_is_unfaithful():
    # Real pipeline; the claim cites ref-1 but ref-1's excerpt shares no
    # significant token with it ("reserves" must not appear in the excerpt).
    content = "Gold reserves increased [ref:ref-1]."
...
def test_claim_citing_ref_a_is_not_supported_by_ref_b():
    # Cite-aware grounding: a claim citing ref-1 is verified ONLY against
    # ref-1's excerpt. ref-2 would support it under full-evidence matching, but
    # it is ignored because the claim did not cite it.
    content = "Gold reserves increased [ref:ref-1]."
    citations = (
        _citation(excerpt="Marketing spend declined sharply."),  # ref-1: no overlap
        EvidenceCitation(
            reference_id="ref-2",
            source="aether.records.ledger.tx-2",
            tenant_id="t-1",
            excerpt="Gold reserves increased markedly.",
        ),
    )
    vresult = VerificationEngine().run(_result(content=content, citations=citations))
...
def test_claim_citing_matching_reference_is_supported():
    # Cite-aware grounding: the claim cites ref-1 and ref-1's excerpt matches,
    # so the check points at ref-1 and passes even when another citation would
    # also match.
    content = "Gold reserves increased [ref:ref-1]."
    citations = (
        EvidenceCitation(
        ...
        _citation(excerpt="Gold reserves increased markedly."),  # ref-1 matches
    )
    vresult = VerificationEngine().run(_result(content=content, citations=citations))
    assert len(vresult.checks) == 1
    assert vresult.checks[0].reference_id == "ref-1"
...
# ---------------------------------------------------------------------------
# Zero-claims edges: NON-empty output with zero claims is REJECTED; empty
# output keeps the leak-only gate
# ---------------------------------------------------------------------------


def test_nonempty_content_with_zero_claims_is_rejected():
    engine = VerificationEngine(
        extractor=_FakeExtractor(claims=[]),
        checker=_FakeChecker(checks=[]),
        leaks=SecretLeakDetector(),
    )
    # Content is non-empty but claim extraction yields zero: a hallucinated
    # numbered list would otherwise pass with nothing to verify. Reject.
    vresult = engine.run(_result())
...
def test_nonempty_content_with_zero_claims_and_leak_is_rejected():
    engine = VerificationEngine(
        extractor=_FakeExtractor(claims=[]),
        checker=_FakeChecker(checks=[]),
        leaks=SecretLeakDetector(),
...
def test_numbered_list_only_content_is_rejected():
    # A hallucinated numbered list is all structural lines: extraction yields
    # zero claims, and a non-empty output with nothing to verify is rejected.
    content = "1. Revenue grew 12%.\n2. Net income doubled.\n3. Costs fell."
    vresult = VerificationEngine().run(_result(content=content, citations=(_citation(),)))
    assert vresult.claims == ()
    assert vresult.checks == ()
    assert vresult.faithful is False
    assert vresult.leak_detected is False
...
def test_empty_content_with_zero_claims_keeps_leak_only_gate():
    engine = VerificationEngine(
        extractor=_FakeExtractor(claims=[]),
        checker=_FakeChecker(checks=[]),
        leaks=SecretLeakDetector(),
    )
    # Empty output yields zero claims and zero checks; all([]) is True, so
    # faithful depends only on leak_detected.
    vresult = engine.run(_result(content=""))
    assert vresult.claims == ()
    assert vresult.checks == ()
    assert vresult.faithful is True
    assert vresult.leak_detected is False
