...
class _EchoSynthesizer:
    ...
    citations (faithful) and is deterministically grounded. With ``cite=True``
    each echoed line carries its inline ``[ref:<id>]`` marker so strict
    cite-aware verification (which attributes each claim ONLY to its cited
    references) can trace every claim back to the citation it cites.
    """

    def __init__(self, *, cite: bool = False) -> None:
    ...
    def _echo(self, prompt: str) -> str:
        ...
        for index, line in enumerate(lines):
            if line.startswith("[ref:") and index + 1 < len(lines):
                ref_id = line[len("[ref:"):].split("]", 1)[0].strip()
                content = lines[index + 1].strip()
                if content and not content.startswith("[ref:"):
                    if self._cite:
                        echoed.append(self._cite_sentence(content, ref_id))
                    else:
                        echoed.append(content)
        return " ".join(echoed)

    @staticmethod
    def _cite_sentence(sentence: str, ref_id: str) -> str:
        """Place the inline ``[ref:<id>]`` marker inside the sentence.

        The marker is inserted before the trailing sentence punctuation so
        claim extraction keeps the marker in the same sentence as the claim it
        cites (a marker placed after the period would be split into the next
        sentence and leave the claim uncited).
        """
        marker = f" [ref:{ref_id}]"
        if sentence.endswith((".", "!", "?")):
            return f"{sentence[:-1]}{marker}{sentence[-1]}"
...
async def test_full_happy_path_synthesize_verify_render():
    ...
    synthesizer = _EchoSynthesizer(cite=True)
