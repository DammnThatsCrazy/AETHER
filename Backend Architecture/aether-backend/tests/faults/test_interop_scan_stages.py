...
from services.interop.scan_worker import (  # noqa: E402
    SENTINEL_NETWORK,
    InteropEventPublisher,
    InteropGraphProjector,
    RpcRateLimited,
    ScanWorker,
)
...
class FakeScanAdapter:
    ...
    Self-contained (the branch-only ``OperationalFieldsMixin`` was dropped): it
    implements the adapter ``scan`` surface the ScanWorker drives plus the
    operational-state view used by the reconciliation persist step.
    """
    ...
    async def scan(self, checkpoint=None):
        """Supervised scan entry: run ``_scan_cycle``; re-raise guards/failures."""
    ...
    def operational_state(self, checkpoint=None):
        ...
        return {
            ...
            "configured": self.rpc is not None,
            "credential_status": "configured" if self.rpc is not None else "credential_waiting",
