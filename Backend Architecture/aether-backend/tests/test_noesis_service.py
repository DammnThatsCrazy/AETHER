...
@pytest.mark.asyncio
async def test_conversation_id_collision_cannot_append_across_tenants(service: NoesisService, tenant: TenantContext):
    await service.query(
        NoesisQueryRequest(message="show unresolved alerts", surface="aether", conversation_id="conv-owned"),
        tenant,
    )
    tenant_b = TenantContext(tenant_id="tenant-b", role=Role.VIEWER, permissions=["read"])
    ...
    record = await service.conversations.get("conv-owned", tenant_id="tenant-a", surface="aether")
    assert record["tenant_id"] == "tenant-a"
    assert len(record["messages"]) == 2


@pytest.mark.asyncio
async def test_streaming_query_emits_error_event_for_guardrail_failure(tenant: TenantContext):
    from types import SimpleNamespace
    ...
    events = []
    async for event in _query_event_stream(
        NoesisQueryRequest(message="show alerts", surface="aether", tenant_id="tenant-b"),
        request,
        GraphClient(),
    ):
        events.append(event)
