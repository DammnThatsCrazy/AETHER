...
class IntelligenceAggregator:
    ...
    async def tier(self, entity_id: str, tenant_id: str, window: str = "30d") -> dict:
        ...
        rows = _filter((await _safe("tier", self._tiers.get_metrics(entity_id, tenant_id=tenant_id))).value_or([]), tenant_id, cutoff)
        items = []
        for r in rows:
            v, d = _val(r), _dims(r)
            items.append({
    ...
    async def asset_composition(self, entity_id: str, tenant_id: str, window: str = "30d") -> dict:
        ...
        cutoff = _window_cutoff(window)
        rows = _filter((await _safe("asset_composition", self._asset_composition.get_metrics(entity_id, tenant_id=tenant_id))).value_or([]), tenant_id, cutoff)
        items = []
        total_value = 0.0
        for r in rows:
            v, d = _val(r), _dims(r)
    ...
    async def pnl(self, entity_id: str, tenant_id: str, window: str = "30d") -> dict:
        ...
        cutoff = _window_cutoff(window)
        rows = _filter((await _safe("pnl", self._pnl.get_metrics(entity_id, tenant_id=tenant_id))).value_or([]), tenant_id, cutoff)
        items = []
        for r in rows:
            v, d = _val(r), _dims(r)
            if isinstance(v, dict) and v:
    ...
    async def trading_profile(self, entity_id: str, tenant_id: str, window: str = "30d") -> dict:
        ...
        cutoff = _window_cutoff(window)
        rows = _filter((await _safe("trading_profile", self._trading_profile.get_metrics(entity_id, tenant_id=tenant_id))).value_or([]), tenant_id, cutoff)
        items = []
        for r in rows:
            v, d = _val(r), _dims(r)
            if isinstance(v, dict) and v:
    ...
    async def location_history(self, entity_id: str, tenant_id: str, window: str = "30d", limit: int = 20) -> dict:
        ...
        cutoff = _window_cutoff(window)
        rows = _filter((await _safe("location_history", self._location_history.get_metrics(entity_id, tenant_id=tenant_id))).value_or([]), tenant_id, cutoff)
        items = []
        for r in rows:
            v, d = _val(r), _dims(r)
            if isinstance(v, dict) and v:
    ...
    async def temporal_heatmap(self, entity_id: str, tenant_id: str, window: str = "30d") -> dict:
        ...
        cutoff = _window_cutoff(window)
        rows = _filter((await _safe("temporal_heatmap", self._temporal_heatmap.get_metrics(entity_id, tenant_id=tenant_id))).value_or([]), tenant_id, cutoff)
        items = []
        for r in rows:
            v, d = _val(r), _dims(r)
            if isinstance(v, dict) and v:
    ...
    async def social_intelligence(self, entity_id: str, tenant_id: str, window: str = "30d") -> dict:
        ...
        cutoff = _window_cutoff(window)
        rows = _filter((await _safe("social_intelligence", self._social.get_metrics(entity_id, tenant_id=tenant_id))).value_or([]), tenant_id, cutoff)
        items = []
        for r in rows:
            v, d = _val(r), _dims(r)
            if isinstance(v, dict) and v:
    ...
    async def journey_economics(self, entity_id: str, tenant_id: str, window: str = "30d", limit: int = 20) -> dict:
        ...
        cutoff = _window_cutoff(window)
        rows = _filter((await _safe("journey_economics", self._journey_econ.get_metrics(entity_id, tenant_id=tenant_id))).value_or([]), tenant_id, cutoff)
        items = []
        for r in rows:
            v, d = _val(r), _dims(r)
            if isinstance(v, dict) and v:
    ...
    async def device_performance(self, entity_id: str, tenant_id: str, window: str = "30d") -> dict:
        ...
        cutoff = _window_cutoff(window)
        rows = _filter((await _safe("device_performance", self._journey_econ.get_metrics(entity_id, tenant_id=tenant_id))).value_or([]), tenant_id, cutoff)
    ...
    async def funnel(
        ...
        cutoff = _window_cutoff(window)
        rows = _filter((await _safe("funnel", self._journey_econ.get_metrics(entity_id, tenant_id=tenant_id))).value_or([]), tenant_id, cutoff)
        if campaign_id:
    ...
    async def time_to_convert(self, entity_id: str, tenant_id: str, window: str = "30d") -> dict:
        ...
        cutoff = _window_cutoff(window)
        rows = _filter((await _safe("time_to_convert", self._journey_econ.get_metrics(entity_id, tenant_id=tenant_id))).value_or([]), tenant_id, cutoff)
    ...
    async def retarget_recommendations(
        ...
        rows = _filter((await _safe("retarget_recs", self._journey_econ.get_metrics(entity_id, tenant_id=tenant_id))).value_or([]), tenant_id, None)
        items = []
        for r in rows:
            d = _dims(r)
    ...
    async def web2(self, entity_id: str, tenant_id: str, window: str = "30d") -> dict:
        ...
        tradfi_read, credit_read = await asyncio.gather(
            _safe("web2.tradfi", self._tradfi_portfolio.get_metrics(entity_id, tenant_id=tenant_id)),
            _safe("web2.credit", self._credit_signals.get_metrics(entity_id, tenant_id=tenant_id)),
        )
    ...
    async def protocol_metrics(self, entity_id: str, tenant_id: str, window: str = "30d") -> dict:
        ...
        cutoff = _window_cutoff(window)
        rows = _filter((await _safe("protocol_metrics", self._web3_metrics.get_metrics(entity_id, tenant_id=tenant_id))).value_or([]), tenant_id, cutoff)
        items = []
        for r in rows:
            v, d = _val(r), _dims(r)
            if isinstance(v, dict) and v:
    ...
    async def governance_activity(
        ...
        cutoff = _window_cutoff(window)
        all_rows = _filter((await _safe("governance_activity", self._governance.get_metrics(entity_id, tenant_id=tenant_id))).value_or([]), tenant_id, cutoff)
        items = []
        for r in all_rows:
            v, d = _val(r), _dims(r)
            if isinstance(v, dict) and v:
