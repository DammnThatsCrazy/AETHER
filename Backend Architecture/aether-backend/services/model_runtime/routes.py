...
* Server-authoritative tenant scope: every route requires the ``X-Tenant-ID``
  request header (the repo's canonical tenant header). A model/client can never
  select tenant scope from the body or query.
...
def require_tenant_id(
    ...
    """Resolve the server-authoritative tenant scope from ``X-Tenant-ID``.

    The gate guard runs first so a disabled surface 503s even without a tenant
    header; an enabled surface with no tenant identity is rejected (HTTP 400).
    A model/client can never select tenant scope — the header is the only input.
    """
    _gate_guard()
...
async def set_tenant_default(
    ...
    here. Unknown model ids are rejected (HTTP 400).
    """
...
async def get_registry(tenant_id: str = Depends(require_tenant_id)) -> RegistryResponseOut:
    """GET /v1/model-runtime/registry — the full model catalog.

    Consumed by the Kyber ``ModelRegistryPage`` (C14). Serves the generated
    registry projected to the frontend shape; no per-tenant data, no
    credentials.
    """
...
async def get_health(tenant_id: str = Depends(require_tenant_id)) -> HealthResponseOut:
    """GET /v1/model-runtime/health — provider health summary.

    Consumed by the Kyber ``ModelRuntimeHealthPage`` (C14). Reasons pass
    through :func:`_sanitize_reason` so secret-shaped material is blanked
    before it can reach the client. Backed by ``RuntimeHealthProbe`` over the
    deterministic seed provider set.
    """
...
async def get_entitlements(
    ...
    Consumed by the Kyber ``EntitlementsPage`` (C14). Server-authoritative:
    rows are resolved by the ``AllowlistEntitlementResolver`` for the
    requesting tenant only; a model can never select tenant scope.
    """
...
async def get_usage(tenant_id: str = Depends(require_tenant_id)) -> UsageResponseOut:
    """GET /v1/model-runtime/usage — aggregate + per-model usage.

    Consumed by the Kyber ``UsagePage`` (C14). Deterministic seed data
    (all-zero, fail-closed) until a metering store is wired; the shape matches
    the Kyber ``UsageResponse`` exactly.
    """
...
async def get_traces(tenant_id: str = Depends(require_tenant_id)) -> TracesResponseOut:
    """GET /v1/model-runtime/traces — routing trace summaries.

    Consumed by the Kyber ``TracesPage`` (C14). Deterministic seed data,
    tenant-scoped, and content-free: only routing-decision summary fields are
    ever returned — never request/response bodies.
    """
