...
class AnthropicModelProvider(BaseModelProvider):
    ...
    def bind_credential(self, resolution, *, materializer=None) -> "AnthropicModelProvider":
    ...
    async def _materialize_secret(self, resolution) -> str:
        ...
        if not callable(materializer):
            raise ModelNotConfigured(
                "anthropic: tenant secret-backend credential requires backend "
                "materialization"
            )
        try:
            ...
            raise ModelNotConfigured(
                "anthropic: tenant secret-backend credential not materializable"
            ) from exc
