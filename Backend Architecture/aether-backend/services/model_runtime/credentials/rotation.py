...
class RotationPolicy(Protocol):
    ...
    async def evaluate(
        ...
        *,
        ...
        Pass ``now`` to pin the clock (deterministic); when ``None`` the
        current UTC time is used (production behavior).
        """
...
class RotationOrchestrator:
    ...
    async def evaluate_all(
        ...
        *,
        ...
        Pass ``now`` to pin the reference clock for every decision
        (deterministic); when ``None`` each policy evaluation uses the current
        UTC time (production behavior).
        """
        decisions: list[RotationDecision] = []
        for ref in refs:
            meta = await self._source.load(tenant_id, ref)
            decisions.append(await self._policy.evaluate(meta, now=now))
        return decisions
