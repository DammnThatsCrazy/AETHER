...
class AgenticIntelligenceAdapter:
    ...
    async def answer(self, *, intent: str, tenant_id: str, target: str | None = None, limit: int = 10) -> AgenticNoesisAnswer:
        ...
        elif intent == "agent_profile360_lookup":
            if not target:
                warnings.append("Agent Profile 360 lookup requires an agent_id target.")
                rows = []
            else:
                profile = await AgenticProductSurfacesService().agent_profile360(
                    tenant_id=tenant_id,
                    agent_id=target,
                    limit=limit,
                )
                rows = [profile]
            sources.extend(self._sources(fetched_at, "agentic_product_surfaces", "canonical_activity"))
            claims.append(self._claim("Agent Profile 360 agentic evidence is deterministically assembled from observed facts.", "deterministic_computation", bool(rows)))
            answer = "Built Agent Profile 360 evidence from tenant-scoped observations." if rows else "I need an agent_id to build Agent Profile 360 evidence."
        elif intent == "journey_agentic_steps_lookup":
            journey = await AgenticProductSurfacesService().journey_v2_agentic_steps(
                tenant_id=tenant_id,
                agent_id=target,
                limit=limit,
            )
            rows = journey.get("steps", [])
            sources.extend(self._sources(fetched_at, "canonical_activity", "agentic_product_surfaces"))
            claims.append(self._claim("Journey v2 agentic steps are observed canonical activity rows.", "observed_fact", bool(rows)))
            answer = f"Found {len(rows)} observed Journey v2 agentic steps."
        elif intent == "campaign_agentic_influence_lookup":
            if not target:
                warnings.append("Campaign agentic influence lookup requires a campaign_id target.")
                rows = []
            else:
                influence = await AgenticProductSurfacesService().campaign_agentic_influence(
                    tenant_id=tenant_id,
                    campaign_id=target,
                    limit=limit,
                )
                rows = [influence]
            sources.extend(self._sources(fetched_at, "canonical_activity", "agentic_product_surfaces"))
            claims.append(self._claim("Campaign influence is eligible only when observed agentic campaign touchpoints exist.", "deterministic_computation", bool(rows)))
            answer = "Summarized agentic campaign influence from observed touchpoints." if rows else "I need a campaign_id to summarize agentic campaign influence."
        else:
