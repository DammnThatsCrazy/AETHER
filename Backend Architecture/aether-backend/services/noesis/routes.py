...
async def _query_event_stream(
    ...
    except Exception as exc:  # noqa: BLE001 - stream clients need an in-band terminal error event
        await _record_audit(
            request,
            ...
            outcome="denied",
            metadata={"surface": body.surface, "stream": True, "error": exc.__class__.__name__},
        )
        yield _sse("error", {"code": exc.__class__.__name__, "message": str(exc) or "Noesis stream failed."})
        return
    await _record_audit(
        request,
        event_type="noesis_query",
