...
def _base_fact(record: AgenticObservationRecord, bronze_id: str, received_at: str) -> dict[str, Any]:
    ...
    return {
        ...
        "agent_id": agent_id,
        "runtime_id": (record.runtime.runtime_id if record.runtime else None) or (record.agent.runtime_id if record.agent else None),
        "connection_id": correlation.connection_id if correlation else None,
        "server_id": mcp.server_identity_hash if mcp else None,
        "tool_id": mcp.tool_id if mcp else None,
        "invocation_id": correlation.invocation_id if correlation else None,
        "task_id": correlation.task_id if correlation else None,
        "trace_id": (correlation.trace_id if correlation else None) or record.observation_id,
        "authorization_id": authorization.authorization_id if authorization else None,
        "external_account_id": authorization.external_account_id if authorization else None,
        "provider_request_id": (verification.provider_request_id if verification else None) or (correlation.provider_request_id if correlation else None),
        "external_object_id": (verification.external_object_id if verification else None) or record.object.external_object_id or (correlation.external_object_id if correlation else None),
        "status": record.action.status,
        "verification_status": (verification.verification_status if verification else None) or "unverified",
        ...
        "evidence_ref": (verification.evidence_ref if verification else None) or f"bronze:{bronze_id}",
    ...
    async def ingest_record(
        ...
        *,
        ...
        activity = CanonicalActivity(
            ...
            session_id=record.correlation.session_id if record.correlation else None,
            campaign_id=record.correlation.campaign_id if record.correlation else None,
