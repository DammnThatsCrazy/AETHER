...
PAYMENT_VOLUME_EVENTS = {StablecoinEventType.PAYMENT, StablecoinEventType.X402_PAYMENT_OBSERVED, StablecoinEventType.SETTLEMENT_OBSERVATION}
EXCLUDED_ORGANIC_VOLUME_EVENTS = {StablecoinEventType.MINT, StablecoinEventType.BURN, StablecoinEventType.BRIDGE_MINT, StablecoinEventType.BRIDGE_BURN}
...
class StablecoinGoldMaterializer:
    ...
    def summarize_finalized_payment_volume(self, rows: Iterable[StablecoinMetricInput], *, window_start: str, window_end: str) -> list[StablecoinGoldMetric]:
        ...
        for row in rows:
            ...
            if row.event_type in EXCLUDED_ORGANIC_VOLUME_EVENTS or row.internal_transfer:
                continue
            key = (row.tenant_id, row.entity_id, row.entity_type, row.money.canonical_asset_id, row.money.deployment_id, row.money.chain_id, row.source)
