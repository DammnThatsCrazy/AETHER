...
This module keeps PR2 observation-first: provider payloads become governed Bronze
records first, then eligible records are normalized into tenant-scoped Silver-like
stablecoin observations. It does not mutate the graph and it does not execute
payments.
"""
...
from .registry import StablecoinDeploymentRegistry, PLATFORM_STABLECOIN_REGISTRY
...
class StablecoinIngestionPipeline:
    ...
    def _normalize(
        ...
        observation = StablecoinObservation(
            ...
            classification_confidence=Decimal("0.95") if obs.event_type != StablecoinEventType.UNKNOWN_STABLECOIN_MOVEMENT else Decimal("0.25"),
        )
