...
class StablecoinGovernanceService:
    ...
    def capability_allowed(self, granted: Iterable[str], required: StablecoinCapabilityEntitlement) -> dict[str, Any]:
        granted_set = set(granted)
        allowed = required.value in granted_set
        return {"capability": required.value, "allowed": allowed, "reason": "granted" if allowed else "missing_capability"}
