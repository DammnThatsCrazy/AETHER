"""Infrastructure360 — read-only projection routes at ``/v1/infrastructure``.

The infrastructure360 projection is a **read-only intelligence projection** over
canonical Aether infrastructure truth. These routes expose that projection as a
tenant-scoped API under the ``/v1/infrastructure`` prefix:

* ``GET /v1/infrastructure/{subject_kind}/{subject_id}`` — run the
  infrastructure360 projection for the requesting tenant. Tenant scope is
  ...
  permission plus the projection's ``infrastructure360.read`` capability key
  ...
  honestly here).
* ``GET /v1/infrastructure/health`` — read-only plane probe: whether the
  infrastructure360 provider is registered and contract-compatible.
...
from services.infrastructure.provider import build_projection_request
...
def _require_infrastructure360_read(request: Request) -> str:
    """Tenant-scoped read gate for the infrastructure360 surface.

    Enforces the canonical tenant read permission and asserts the projection's
    ``infrastructure360.read`` capability key is genuinely declared for the
...
def _registry_health(registry: Any) -> dict[str, Any]:
    """Read-only plane probe for the infrastructure360 provider.
...
def create_router(registry: Any = projection_registry) -> APIRouter:
    """Build the ``/v1/infrastructure`` router bound to ``registry``.
    ...
    """
    r = APIRouter(prefix="/v1/infrastructure", tags=["Infrastructure 360"])

    @r.get(
        "/{subject_kind}/{subject_id}",
        summary="Run the Infrastructure360 projection for the requesting tenant",
    )
    async def get_infrastructure_projection(
        ...
        tenant_id = _require_infrastructure360_read(request)
        ...
        try:
            ...
            raise HTTPException(
                status_code=503,
                detail="infrastructure360 projection unavailable",
            ) from exc
        return result.model_dump(mode="json")

    @r.get(
        "/health",
        summary="Infrastructure360 provider plane probe (read-only)",
    )
