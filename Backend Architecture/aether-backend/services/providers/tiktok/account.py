"""TikTok Shop account discovery and selection (:class:`AccountAdapter`).

TikTok Shop orders are scoped to ONE shop per credential: the ``shop_id`` the
app credential is scoped to. Discovery returns a single :class:`ProviderAccount`
...
from services.providers.tiktok.auth import _credential_dict
...
def _shop_account(shop_id: str) -> ProviderAccount:
    ...
    return ProviderAccount(
        ...
        display_name=f"TikTok Shop {shop_id}",
        external_id=shop_id,
        currency=None,
...
__all__ = ["TikTokAccountAdapter", "_shop_account"]
