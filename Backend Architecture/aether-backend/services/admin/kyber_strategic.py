...
class IntegrationHealth(BaseModel):
    ...
    dispatch_failure_rate: float
    retry_rate: float
    integration_count: int
    premium_connector_events: int
    manual_action_count: int
    integrated_action_count: int
    manual_to_integrated_ratio: float
    stale_approved_actions: int
    action_to_outcome_conversion_rate: float
    revenue_metering_events: int
    estimated_billable_value: float
    recommended_olympus_action: str


class KyberStrategicObservability:
    def __init__(self, tenants: list[dict[str, Any]], recommendations: list[dict[str, Any]], decisions: list[dict[str, Any]], actions: list[dict[str, Any]], outcomes: list[dict[str, Any]], feedback: list[dict[str, Any]], playbooks: list[dict[str, Any]], runs: list[dict[str, Any]], window: Window = "30d", integrations: list[dict[str, Any]] | None = None, dispatches: list[dict[str, Any]] | None = None, receipts: list[dict[str, Any]] | None = None, metering: list[dict[str, Any]] | None = None) -> None:
        self.window = window
        ...
        self.integrations = [item for item in (integrations or []) if _within_window(item, window)]
        self.dispatches = [item for item in (dispatches or []) if _within_window(item, window)]
        self.receipts = [item for item in (receipts or []) if _within_window(item, window)]
        self.metering = [item for item in (metering or []) if _within_window(item, window)]
    ...
    def data_freshness(self) -> dict[str, Any]:
        timestamps = [_record_time(item) for collection in (self.recommendations, self.decisions, self.actions, self.outcomes, self.feedback, self.runs, self.dispatches, self.receipts, self.metering) for item in collection]
        timestamps = [item for item in timestamps if item is not None]
        newest = max(timestamps).isoformat() if timestamps else None
        return {"status": "fresh" if timestamps else "empty", "newest_record_at": newest, "record_count": sum(len(c) for c in (self.recommendations, self.decisions, self.actions, self.outcomes, self.feedback, self.runs, self.dispatches, self.receipts, self.metering))}
    ...
    def tenant_ids(self) -> set[str]:
        ...
        ids |= {_tenant_id(item) for collection in (self.recommendations, self.decisions, self.actions, self.outcomes, self.playbooks, self.runs, self.integrations, self.dispatches, self.metering) for item in collection}
        ids.discard("unknown")
        return ids
    ...
    def integration_health(self) -> list[IntegrationHealth]:
        ...
        outcomes_by_action = {str(outcome.get("action_id")) for outcome in self.outcomes}
        for tenant_id in sorted(self.tenant_ids()):
            actions = [item for item in self.actions if _tenant_id(item) == tenant_id]
            dispatches = [item for item in self.dispatches if _tenant_id(item) == tenant_id]
            integrations = [item for item in self.integrations if _tenant_id(item) == tenant_id]
            metering = [item for item in self.metering if _tenant_id(item) == tenant_id]
            delivered = sum(1 for item in dispatches if item.get("status") == "delivered")
            failed = sum(1 for item in dispatches if item.get("status") == "failed")
            retries = sum(1 for item in metering if item.get("event_type") == "integration_retry")
            premium = sum(1 for item in metering if item.get("event_type") == "premium_connector_used")
            integrated_action_ids = {str(item.get("action_id")) for item in dispatches}
            manual_actions = [item for item in actions if str(item.get("action_id")) not in integrated_action_ids]
            stale = self._stale_approved_actions(actions, dispatches)
            conversion = _rate(sum(1 for action_id in integrated_action_ids if action_id in outcomes_by_action), len(integrated_action_ids))
            failure_rate = _rate(failed, len(dispatches))
            recommended = "Monitor integration adoption and collect workflow value evidence."
            if len(manual_actions) > len(integrated_action_ids):
                recommended = "Recommend integration upsell for high manual action volume."
            if failure_rate >= 0.25:
                recommended = "Recommend implementation support for failed dispatches."
            if premium >= 3:
                recommended = "Recommend higher tier packaging for premium connector usage."
            if stale >= 3:
                recommended = "Recommend operational review for stale approved actions."
            rows.append(IntegrationHealth(
                tenant_id=tenant_id,
                dispatch_volume=len(dispatches),
                dispatch_success_rate=_rate(delivered, len(dispatches)),
                dispatch_failure_rate=failure_rate,
                retry_rate=_rate(retries, len(dispatches)),
                integration_count=len(integrations),
                premium_connector_events=premium,
                manual_action_count=len(manual_actions),
                integrated_action_count=len(integrated_action_ids),
                manual_to_integrated_ratio=_rate(len(manual_actions), len(integrated_action_ids) or 1),
                stale_approved_actions=stale,
                action_to_outcome_conversion_rate=conversion,
                revenue_metering_events=len(metering),
                estimated_billable_value=round(sum(_num(item.get("estimated_billable_value")) for item in metering), 2),
                recommended_olympus_action=recommended,
            ))
        return rows

    def _stale_approved_actions(self, actions: list[dict[str, Any]], dispatches: list[dict[str, Any]]) -> int:
        dispatched = {str(item.get("action_id")) for item in dispatches}
        cutoff = datetime.now(timezone.utc) - timedelta(days=7)
        return sum(1 for action in actions if action.get("status") in {"planned", "queued"} and str(action.get("action_id")) not in dispatched and (_record_time(action) or datetime.now(timezone.utc)) < cutoff)

    def revenue_opportunities(self) -> list[RevenueOpportunity]:
        opportunities: list[RevenueOpportunity] = []
        for integration in self.integration_health():
            if integration.manual_action_count > integration.integrated_action_count and integration.manual_action_count >= 2:
                opportunities.append(RevenueOpportunity(
                    opportunity_id=str(uuid.uuid5(uuid.NAMESPACE_URL, f"{integration.tenant_id}:integration:{self.window}")),
                    tenant_id=integration.tenant_id,
                    opportunity_type="integration_expansion",
                    reason="Tenant has high manual action volume relative to integrated dispatches.",
                    supporting_metrics=integration.model_dump(include={"manual_action_count", "integrated_action_count", "dispatch_success_rate"}),
                    estimated_value=round(max(integration.manual_action_count * 250, 1000), 2),
                    confidence=0.68,
                    recommended_action="Recommend integration upsell to convert manual actions into dispatched workflows.",
                    created_at=self.generated_at,
                ))
            if integration.dispatch_failure_rate >= 0.25 and integration.dispatch_volume > 0:
                opportunities.append(RevenueOpportunity(
                    opportunity_id=str(uuid.uuid5(uuid.NAMESPACE_URL, f"{integration.tenant_id}:dispatch-support:{self.window}")),
                    tenant_id=integration.tenant_id,
                    opportunity_type="services_expansion",
                    reason="Tenant has elevated dispatch failure rate.",
                    supporting_metrics=integration.model_dump(include={"dispatch_volume", "dispatch_failure_rate", "retry_rate"}),
                    estimated_value=1500,
                    confidence=0.7,
                    recommended_action="Recommend implementation support for integration reliability.",
                    created_at=self.generated_at,
                ))
