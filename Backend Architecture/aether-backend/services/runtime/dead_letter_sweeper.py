...
- **Payment dead-lettered receipts** (``ReceiptState.DEAD_LETTERED``) — replayed
  per tenant through :meth:`PaymentRailsService.replay_dead_lettered`, which
  resets the bounded repair counter and re-drives ONE idempotent canonical-repair
  pass so replayed deliveries actually progress.
...
async def _replay_payment_dead_letters(limit: int) -> dict[str, int]:
    """Replay dead-lettered payment receipts per tenant (idempotent repair)."""
    ...
    for tenant_id in sorted(tenants_with_dead)[: max(1, limit)]:
        try:
            result = await service.replay_dead_lettered(
                tenant_id, actor="dead_letter_sweeper", limit=per_tenant_limit
            )
            replayed += int(result.get("count", 0))
