...
class StablecoinDeploymentRepository(BaseRepository):
    def __init__(self) -> None: super().__init__("stablecoin_deployments")

class StablecoinObservationRepository(BaseRepository):
    def __init__(self) -> None: super().__init__("stablecoin_observations")
    @staticmethod
    def observation_key(record: dict[str, Any]) -> str:
        ...
        if missing: raise ValueError(f"missing stablecoin observation identity: {','.join(missing)}")
        idx = record.get("log_or_instruction_index", "")
        return hashlib.sha256(":".join(str(record[k]) for k in required + []) .encode() + f":{idx}".encode()).hexdigest()[:32]
    async def upsert_observation(self, record: dict[str, Any]) -> dict[str, Any]:
        if not record.get("tenant_id"): raise ValueError("tenant_id is required")
        ...
        if existing: return await self.update(record_id, {**existing, **data})
...
class StablecoinSupportAssertionRepository(BaseRepository):
    def __init__(self) -> None: super().__init__("stablecoin_support_assertions")

class StablecoinReconciliationRepository(BaseRepository):
    def __init__(self) -> None: super().__init__("stablecoin_reconciliation_results")
...
class StablecoinGoldIdentity:
    ...
    def key(*, tenant_id: str, metric_name: str, metric_version: str, entity_id: str, entity_type: str, canonical_asset_id: str, deployment_id: str, chain_id: str, window_start: str, window_end: str, dimensions: Optional[dict[str, Any]] = None, source: str = "") -> str:
        if not tenant_id: raise ValueError("tenant_id is required for tenant-owned Gold stablecoin metrics")
...
class StablecoinGoldRepository(BaseRepository):
    def __init__(self) -> None: super().__init__("gold_stablecoin_metrics")
    async def materialize_metric(self, **kwargs: Any) -> dict[str, Any]:
        ...
        return await self.update(metric_id, {**existing, **data}) if existing else await self.insert(metric_id, data)
