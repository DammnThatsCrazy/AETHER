...
resource "aws_cloudwatch_log_group" "tfmcp" {
  ...
  tags = {
    Name    = lower("${var.project}-${var.environment}-${local.service_key}-logs")
    Purpose = "tfmcp MCP server logs"
  }
}
...
resource "aws_secretsmanager_secret" "tfmcp_auth" {
  ...
  tags = {
    Name    = lower("${var.project}-${var.environment}-tfmcp-auth")
    Purpose = "MCP server auth token"
  }
}
...
resource "aws_secretsmanager_secret" "tfmcp_github_pat" {
  ...
  recovery_window_in_days = 30
  tags = {
    Name    = lower("${var.project}-${var.environment}-tfmcp-github-pat")
    Purpose = "GitHub PAT"
  }
}
...
resource "aws_iam_role" "tfmcp_execution" {
  ...
  tags = {
    Name    = lower("${var.project}-${var.environment}-${local.service_key}-execution-role")
...
resource "aws_iam_role" "tfmcp" {
  ...
  tags = {
    Name    = lower("${var.project}-${var.environment}-${local.service_key}-role")
    Purpose = "ECS task role for tfmcp"
  }
}
...
data "aws_iam_policy_document" "tfmcp_apply" {
  ...
  statement {
    sid       = "DenyIAMWrite"
    effect    = "Deny"
    actions   = ["iam:Put*", "iam:Create*", "iam:Delete*", "iam:Attach*", "iam:Detach*", "iam:Update*", "iam:SetDefaultPolicyVersion"]
    resources = ["*"]
  }
  statement {
...
resource "aws_ecs_task_definition" "tfmcp" {
  ...
  tags = {
    Name    = lower("${var.project}-${var.environment}-${local.service_key}-task")
    Purpose = "tfmcp task definition"
  }
}
...
resource "aws_ecs_service" "tfmcp" {
  ...
  tags = {
    Name    = lower("${var.project}-${var.environment}-${local.service_key}-service")
    Purpose = "tfmcp MCP server"
  }
...
resource "aws_lb_target_group" "tfmcp" {
  ...
  tags = {
    Name    = lower("${var.project}-${var.environment}-${local.service_key}-tg")
    Purpose = "tfmcp target group"
  }
}
...
resource "aws_lb_listener_rule" "tfmcp" {
  ...
  tags = {
    Name    = lower("${var.project}-${var.environment}-${local.service_key}-rule")
    Purpose = "tfmcp ALB listener rule"
  }
}
